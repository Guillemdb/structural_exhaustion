...
\title{Curvature Compression and the Power-of-Two Cycle Problem\\
\large A minimal-counterexample analysis}
...
\begin{abstract}
We give a minimal-counterexample proof of the Erd\H{o}s--Gy\'arf\'as
power-of-two cycle problem.  The $P_{13}$-free theorem of Hegde, Sandeep,
and Shashank is used as a black box, and auxiliary dense-window quiet-block
bookkeeping is recorded in the appendix.  After replacing the power-of-two-cycle predicate by the
absence of edge-rooted Mersenne returns, minimality and a boundaried
replacement principle make every proper atom target-uncompressible; the
$P_{13}$-free theorem then forces a maximal packing of induced $P_{13}$
windows with low density, leaving a large $P_{13}$-free remainder.  The
curvature-rank branch routes rank loss to target defect, compression, or
delocalization; the whole-graph case is handled by exact closed response
profiles, so empty-context data cannot silently identify curvature tests.  The
final closure is driven by two-budget entropy routing and a large-budget branch governed by a surplus-adjusted
...
spine, the assigned surplus consumes the global
surplus budget $\sigma(G)=O(\sqrt n)$; hence all Type B bridge residuals
together carry only $o(|R|)$ negative net charge and cannot support the linear
large-budget deficit in that spine.
The route-8 residual is quantified: if it carries the large-budget charge deficit,
it contains linearly many unpaid silent target-complete-minimal trace-basin
entries, counted by their routed vertices.  The carrier ledger passes each entry
to its canonical minimal target-complete carrier core inside the declared
$u$-supported response algebra, closes zero/one-essential-core entries by exits
(4)--(7), and reduces any surviving large-budget route-8 branch to a
two-carrier route-8 entry.  The terminal obstruction is then isolated as a
single local no-go theorem, \cref{thm:typeA-two-carrier-nogo}, with the
carrier-deletion witnesses recorded as part of the residual data.  All numerical
constants are displayed explicitly, and the finite curvature enumerations are
reproduced, with code, in an appendix.
\end{abstract}
...
\cref{lem:typeB-exclusion} and \cref{prop:typeB-bridge-reduction} close the
...
\cref{def:baseline-spine-demand} & ENT\slash SURP & [129] \texttt{(baseline)} & inv 14, 15; \cref{lem:surviving-active-family} & Fixes the \(O(n)\)-deficit baseline demand used by the surplus accounting branch. & Supplies the common demand baseline for the later sandwich. \\
...
\cref{lem:typeB-bridge-deficit-bound}, \cref{prop:typeB-bridge-sublinear} & EXC & [84] \texttt{(fanmass)} & inv 4, 14, 16, 24, 35; \cref{def:near-cubic-spine}; \cref{lem:typeB-hybrid-B1}, \cref{def:typeB-bridge-statements} & Charges residual centers and B2 failures to high-degree fan-center surplus and obtains \(M_B\le16\sigma(G)=o(|R|)\). & Prevents Type B bridge residuals from carrying the linear large-budget deficit on the derived spine branch. \\
...
\section{Minimality and replacement}
...
|E(Q)|\notin\{2^j-2:j\ge2\}.
\]
If \(Q\) is not the single central edge \(sh\), then after deleting a possible
initial use of the shoulder edge \(a_i b_i\), the path \(Q\) uses a
noncentral shoulder-completion incidence of one of the two shoulders of
\(p_i\).
\end{lemma}
...
If \(Q\) has length \(1\), then \(Q=sh\) and the return is central.  Suppose
\(|E(Q)|\ge2\).  If the first edge of \(Q\) is not \(ss_i'\), where \(s_i'\) is
the other shoulder, then it is a noncentral shoulder-completion incidence at
\(s\).  If the first edge is \(ss_i'\), delete that initial edge.  The next
edge cannot return to \(s\), because \(Q\) is simple, and cannot go to \(x_i\),
because \(Q\subseteq G-x_i\).  Hence the next edge is either the central edge
\(s_i'h\) or a noncentral shoulder-completion incidence at \(s_i'\).  This is
...
For every selected port \(p=(h,x)\in\mathcal P_{\rm exc}\), there is a
canonical finite response support \(\Gamma(p)\) with the following properties.
\begin{enumerate}[label=(\alph*), leftmargin=2.2em]
...
\item If \(p\) is open, then \(\Gamma(p)\) contains the lexicographically first
path \(Q_p\subseteq G-x\) supplied by
\cref{lem:single-open-port-suppression-witness}; this path joins \(a_p\) to
\(b_p\) and has length \(2^{j(p)}-1\) for some \(j(p)\ge2\).
\item If \(p\) is triangular, then \(\Gamma(p)\) contains the triangle
\[
x a_p b_p x
\]
and the lexicographically first return path \(R_p\) from (b).
\end{enumerate}
The construction of \(\Gamma(p)\) uses only \(G\), the ordered vertex labels,
and the port \(p\); hence it is canonical.
\end{lemma}
...
\(G-hx\).  Choose the lexicographically first such path and call it \(R_p\).
...
choose the lexicographically first such path.  If \(p\) is triangular, then
\(a_pb_p\in E(G)\), so \(xa_pb_px\) is a triangle.  In that case record this
triangle together with \(R_p\).  These rules determine a finite subgraph
\(\Gamma(p)\) canonically from \(G\) and \(p\).
\end{proof}
...
\]
equipped with the canonical data \(T(p)\), \(R_p\), and \(\Gamma(p)\) from
...
\(\{p,q\}\subseteq\mathcal A_0\) is one of the following concrete objects.
\begin{enumerate}[label=(\alph*), leftmargin=2.2em]
\item a vertex or edge-incidence contained in both declared demand supports
\[
\bigl(T(p)\cup \Gamma(p)\bigr)
\cap
\bigl(T(q)\cup \Gamma(q)\bigr);
\]
\item a common vertex or common edge-incidence of the two canonical return
paths \(R_p\) and \(R_q\);
\item a common shoulder endpoint or shared cubic buffer vertex among
\(\{a_p,b_p,x(p),a_q,b_q,x(q)\}\);
\item a boundary-degree-profile coordinate which prevents a quotient or
replacement from staying in a single fibre, in the sense of
\cref{lem:degree-profile-fibres};
\item a target-response coordinate witnessing a target-defective quotient,
target-complete compression, or delocalization event, in the sense of
\cref{lem:context-universality,cor:uncompressible,lem:proper-smearing,lem:no-silent-global-smearing};
\item an arithmetic chord-set obstruction from a suppressed open-port family,
namely the concrete set \(\mathcal S\) of added shoulder chords in
...
lexicographic order of their declared labels and supports, and chord sets by the
lexicographic order of their increasing list of added shoulder chords.
...
defined as follows.
\begin{enumerate}[label=(\alph*), leftmargin=2.2em]
\item If \(B\) is a vertex, then \(\kappa(B)=B\).
\item If \(B\) is an edge-incidence, then \(\kappa(B)=B\).
\item If \(B\) is a common shoulder endpoint or cubic buffer vertex, then
\(\kappa(B)=B\).
\item If \(B\) is a boundary-degree-profile coordinate or target-response
coordinate, then \(\kappa(B)\) is the least vertex, edge-incidence, or selected
surplus port contained in the declared support of that coordinate.  Paths and
windows contribute their least contained vertex or edge-incidence.  This carrier
exists because every sparse pair-response coordinate is supported on
\(X_\pi\supseteq T(p)\cup T(q)\), and the declared signature of
\cref{def:declared-coordinate-signature} generates quotient coordinates only
from finite unions of declared supports.
\item If \(B\) is a suppressed-family chord-set obstruction \(\mathcal S\), then
...
For a sparse surplus blocker \(B\), let \(\operatorname{supp}(B)\) be its
declared support: the singleton vertex for a vertex blocker, the marked edge
for an edge-incidence blocker, the declared support of a boundary-profile or
target-response coordinate, and the union of the shoulder chords for a
chord-set blocker.  Order the unordered pairs in \(\Pi_{\rm blk}\)
...
lexicographically first connected subgraph of \(G\) with the minimum possible
number of vertices that contains
\[
...
For a set \(\Pi\subseteq\binom{\mathcal A_0}{2}\), let \(X_\Pi\) be the
lexicographically first connected subgraph of \(G\) with the minimum possible
number of vertices that contains every \(X_\pi\) with \(\pi\in\Pi\).  Each
\(r_\pi\) is then viewed as a labelled coordinate of
...
Z(\pi;t,r)
\]
is the finite support generated by the declared signature
\cref{def:declared-coordinate-signature} from the following data:
the capacity token \(t\), the canonical blocker \(\Phi_{\rm can}(\pi)\), the
selected port supports \(T(p),T(q)\), and the canonical response supports
\(\Gamma(p),\Gamma(q)\).

A \emph{routing germ} of \(\pi\) at \(t\) is one of the declared connector germs
in \(Z(\pi;t,r)\) which starts at the primitive carrier of \(t\) and ends at
one of the two selected port supports \(T(p)\) or \(T(q)\).  If two routing
germs have a maximal common initial segment and then leave through two distinct
next incidences, the last vertex of the common initial segment is their
\emph{first separator}.  If no such vertex exists before both germs enter the
same declared selected-port support with the same endpoint label, the two germs
are \emph{parallel}.
...
Consider the two routing germs from the carrier of \(t\) toward those two
selected demands.  If the germs are parallel, then the two declared response
coordinates have the same bounded support data and lie in the same
boundary-degree fibre.  Identifying them is a nontrivial quotient of declared
coordinates.  If some outside context distinguishes the two responses, the
quotient is target-defective by \cref{lem:context-universality}, which is a
...
\section{Induced \texorpdfstring{$P_{13}$}{P13} windows}
...
This is Theorem 1 of Hegde, Sandeep, and Shashank \cite{HSS2024}.
\end{proof}
...
\section{The \texorpdfstring{$P_{13}$}{P13} curvature algebra}
...
target-complete.  Put
\[
\alpha_{\mathcal X}(\xi)=|\mathcal C_{\rm ess}(\xi)|.
\]

An essential carrier for $\xi$ is \emph{private for $\xi$} inside the collection
$\mathcal X$ if it is not essential for any other indexed route-8 entry in the
collection.  Let
\[
\pi_{\mathcal X}(\xi)
\]
be the number of private essential carriers of $\xi$.

The entry $\xi$ is a \emph{two-carrier route-8 entry} if
\[
\pi_{\mathcal X}(\xi)\le2.
\]
...
Let \(w\) be a saturated receiver in a Type A support \(X\).  In the route-8
branch, fix the ambient route-8 Type A collection \(\mathcal X\) containing
\(X\); outside that branch, clauses \textup{(Q1)}--\textup{(Q4)} below define
the same family without using \(\mathcal X\).  The family
\(\mathcal Q_4(w;\mathcal X)\), abbreviated \(\mathcal Q_4(w)\) when the
ambient collection is fixed, consists of boundary-degree-preserving response
quotients generated by the following canonical constructions.
...
\(\xi=(X,w,u,B_u)\in\Xi(\mathcal X)\), the carrier-deletion quotient
\[
\rho_u(B_u)|_{\mathcal C_{\rm ess}(\xi)}
\longrightarrow
\rho_u(B_u)|_{\mathcal C_{\rm ess}(\xi)\setminus\{c\}}
\]
when \(c\in\mathcal C_{\rm ess}(\xi)\), the entry is two-carrier
\(\pi_{\mathcal X}(\xi)\le2\), and the distinguishing event is recorded by a
declared deletion witness in the sense of
\cref{def:typeA-carrier-deletion-witness}.
\end{enumerate}
Every member of \(\mathcal Q_4(w)\) retains the boundary degree profile of the
response state on which it is defined.  Exit (4) occurs precisely when one of
these listed canonical quotients is target-defective.  The declared routed-load
...
Let \(\mathcal X\) be a collection of Type A supports whose saturated receivers
survive only through route 8, and let
\[
\xi=(X,w,u,B_u)\in\Xi(\mathcal X)
\]
be an indexed route-8 entry.  The entry \(\xi\) is a \emph{true route-8
residual entry} if:
...
with exit (4) interpreted through the canonical family
\(\mathcal Q_4(w)\);
...
\end{enumerate}
A \emph{true route-8 residual collection} is a route-8 Type A collection whose
indexed entries are true route-8 residual entries.  Thus, in a true route-8
residual entry, any occurrence of one of exits (1)--(7) contradicts the
...
Let \(\mathcal X\) be a route-8 Type A collection, let
\[
\xi=(X,w,u,B_u)\in\Xi(\mathcal X)
\]
be an indexed entry with \(\pi_{\mathcal X}(\xi)\le2\), and let
\(c\in\mathcal C_{\rm ess}(\xi)\).  If \(c\) has a declared \(c\)-deletion
witness, then \(q_{\xi,c}\in\mathcal Q_4(w)\).
\end{lemma}
...
hypothesis \(\pi_{\mathcal X}(\xi)\le2\) says that \(\xi\) is a two-carrier
route-8 entry.  The declared witness hypothesis supplies the final datum
required in clause (Q5) of \cref{def:typeA-exit4-family}.  Therefore
\(q_{\xi,c}\) is one of the canonical exit-(4) quotients at \(w\).
\end{proof}
...
\xi=(X,w,u,B_u)\in\Xi(\mathcal X)
\]
be a two-carrier indexed route-8 entry, and put
\[
\mathcal C=\mathcal C_{\rm ess}(\xi).
\]
...
\(\mathcal Q_4(w)\).  The preceding paragraph proves that this canonical
...
Let $\xi=(X,w,u,B_u)$ be an indexed route-8 entry.  If
\[
\alpha_{\mathcal X}(\xi)\le1,
\]
...
\item \(\mathcal X\) is a large-budget route-8 Type A collection carrying the
large-budget Type A deficit of \cref{def:typeA-large-budget-deficit}.
...
\[
\alpha_{\mathcal X}(\xi)=|\mathcal C_{\rm ess}(\xi)|\ge2 .
\]
...
other indexed route-8 entries.  Thus it is exactly the residual produced by
\cref{prop:typeA-route8-carrier-reduction}; it is not restricted to the special
case \(|\mathcal C_{\rm ess}(\xi)|=2\).
\end{definition}
...
\begin{proof}
Suppose that \((\mathcal X,\xi)\) is a terminal two-carrier route-8
...
quotient lies in \(\mathcal Q_4(w)\).  Therefore
...
\node[ok] at (-5.08,-1.78) {\(\xi=(X,w,u,B_u)\)\\true route-8 residual\\\(\alpha_{\mathcal X}(\xi)\ge2,\ \pi_{\mathcal X}(\xi)\le2\)};

\node[witness] (wit) at (-0.15,1.05) {declared \(c\)-deletion\\witness};
\node[quot] (q) at (-0.15,-0.30) {canonical quotient\\\(Q_c\in\mathcal Q_4(w)\)};
...
\node[exit] (forced) at (4.05,1.20) {\(\mathcal Q_4(w)\)\\realizes exit (4)};
...
\node[anchor=west,align=left,text width=4.25cm] at (2.98,-1.78) {\cref{lem:typeA-two-carrier-deletion-canonical} gives \(Q_c\in\mathcal Q_4(w)\),\\and \cref{lem:typeA-carrier-deletion-exit} turns it into exit (4).};
\end{tikzpicture}%
}
\caption[Visual proof of the terminal two-carrier route-8 no-go]{Visual proof of the terminal two-carrier route-8 no-go.  Panel I shows a terminal entry \(\xi=(X,w,u,B_u)\).  The grey rectangle is the Type A support \(X\), the blue dashed region is the target-complete-minimal basin \(B_u\), the orange vertex is the saturated receiver \(w\), and the red boundary nodes are essential carrier incidences.  The displayed terminal data are \(\alpha_{\mathcal X}(\xi)\ge2\), \(\pi_{\mathcal X}(\xi)\le2\), and true route-8 residuality.  Panel II isolates one essential carrier \(c\).  Its declared deletion witness has declared support and carrier support, and the two-carrier deletion lemma places the corresponding quotient in the canonical exit-(4) family \(\mathcal Q_4(w)\).  Panel III records the contradiction: the carrier-deletion exit lemma realizes exit (4) at \(w\), while true route-8 residuality says that exits (1)--(7), including exit (4), are absent.  Thus the terminal two-carrier obstruction of \cref{def:typeA-terminal-two-carrier} cannot occur.}
...
Type A deficit.  By \cref{prop:typeA-route8-carrier-reduction}, \(\mathcal X\)
contains an indexed two-carrier entry
\[
\xi=(X,w,u,B_u)
\]
with \(\pi_{\mathcal X}(\xi)\le2\).  Since the collection survives only through
route 8, \(\xi\) is a true route-8 residual entry in the sense of
\cref{def:typeA-true-route8-residual}; in particular, exits (1)--(7) are absent.
By
...
\cref{lem:typeA-essential-deletion-witness,lem:typeA-deletion-witness-declared}
supplies the required declared deletion witness.  Therefore \((\mathcal X,\xi)\) is a terminal
two-carrier route-8 obstruction, contradicting
\cref{thm:typeA-two-carrier-nogo}.  Hence no such collection \(\mathcal X\)
exists.
\end{proof}
...
carrier has a declared deletion witness, the two-carrier condition places the
corresponding deletion quotient in the canonical family \(\mathcal Q_4(w)\),
...
incidence carriers.  If no choice of the local B1 entry containing these
carriers and the other assigned Type B fan entries is pairwise disjoint, then
B2 has a disjoint-carrier failure, which is recorded as a Type B overlap
obstruction by
...
The preceding argument is local to the fan at \(h\).  In the ambient assigned
Type B support, the other fan demands may have several candidate entries.  If
no selection of those entries together with the local B1 entry for \(h\) is
pairwise disjoint, then the global disjoint-carrier clause B2 fails.  By
\cref{lem:typeB-bridge-to-overlap}, such a failure is represented by a minimal
...
fan-window pair $(u,v)$ is not direct-cycle-free, then the envelope contains a
power-of-two cycle.
\end{lemma}
...
non-window incidences by $I_N(\mathfrak F)$.  Each such incidence carries
half-credit in the hybrid fan ledger: packed-window incidences use the
window-incidence carrier of \cref{def:typeB-ledger-carriers}, and non-window
incidences use the local internal/mixed reserve carrier in the same definition.
...
\begin{lemma}[Hybrid B1 fan ledger]\label[lemma]{lem:typeB-hybrid-B1}
Every admissible positive-deficit Type B fan-window residual satisfies the
hybrid B1 alternative of \cref{def:typeB-bridge-statements}: after the direct
cycle, target-defect, compression, and delocalization alternatives are excluded,
the packed-window incidences and the local internal/mixed incidence reserve
form a fan ledger entry whose total capacity is at least
$D_B(\mathfrak F)$.
\end{lemma}

\begin{proof}
The excluded target-defect, compression, and delocalization alternatives are
part of the definition of a positive-deficit residual.  If one of the direct
fan-window cycle conclusions occurs, the first alternative in B1 holds.  In the
remaining case, \cref{lem:typeB-hybrid-incidence-budget} supplies a disjoint
hybrid incidence entry of capacity at least $D_B(\mathfrak F)$.  This is exactly
the hybrid B1 ledger entry.
\end{proof}
...
\item If the fan at \(h\) is not certificate-marked, then \(h\) is a
fan-certificate residual center.  If the local B1 entries cannot be made
disjoint from the other Type B entries in the assigned support, then the support
contains a minimal Type B overlap obstruction.  These are exactly the two Type B
bridge residual alternatives of \cref{def:typeB-bridge-statements}.
\end{enumerate}
\end{proposition}
...
If the fan is not certificate-marked, then the definition of
\cref{def:marked-typeB-fan} makes \(h\) a fan-certificate residual center.  If
the fan is certificate-marked and the local B1 entries exist but cannot be made
globally disjoint inside the assigned support, then the disjoint-carrier clause
B2 in \cref{def:typeB-bridge-statements} fails.  By
\cref{lem:typeB-bridge-to-overlap}, that failure contains a minimal Type B
overlap obstruction.  These two alternatives are exactly the two cases in the
definition of a Type B bridge residual, proving (d).
\end{proof}
...
certificate-marked, or if the B1 entries cannot be made globally disjoint, the
remaining Type B outcome is a fan-certificate residual center or a B2 overlap
obstruction.
\end{corollary}
...
The \emph{overlap support} $Z(\mathcal O)$ is the connected support induced by
the fan centers in $\mathcal D$, all vertices and incidences appearing in their
candidate entries, and the paths or window segments that witness the
corresponding fan-safe relations.  If this set is disconnected, each connected
component is treated as a separate overlap obstruction.
\end{definition}

\begin{lemma}[Finite completion of the Type B support assignment]\label[lemma]{lem:typeB-maximal-completion}
Let $X$ be a connected assigned Type B support with no fan-certificate residual
center.  Suppose that no finite family of Type B demands in \(X\) forms a Type B
overlap obstruction in the sense of \cref{def:typeB-overlap-obstruction}.  Then
the Type B demands of \(X\) admit a maximal disjoint refined Type B ledger.
\end{lemma}

\begin{proof}
Let \(\mathcal M\) be the finite set of vertices of \(X\) that are either
high-degree centers or centers of decorated Type B handoffs in the residual
core determined by the canonical support assignment.  Since \(X\) is finite,
\(\mathcal M\) is finite.  Start with the currently declared Type B demand
family.  If its disjoint refined ledger is not maximal, then some vertex of
\(\mathcal M\) has not yet been entered as a demand center.  Add the
lexicographically first such vertex to the demand family, using the candidate
entries supplied by the Type B assignment rules and by
\cref{lem:typeA-saturated-handoff} for decorated handoff centers.

After adding finitely many demands, absence of a Type B overlap obstruction
means precisely that the enlarged finite demand family has a choice of
candidate entries with pairwise disjoint carriers.  Rechoose the whole ledger
for the enlarged family if necessary.  Each step marks one previously
unprocessed vertex of \(\mathcal M\), and no step creates a new vertex of
\(\mathcal M\).  Hence after at most \(|\mathcal M|\) steps no such vertex
remains.  The resulting disjoint refined ledger is maximal by the definition of
\cref{def:typeB-candidate-ledger}.
\end{proof}
...
has pairwise disjoint carriers.  Among all nonempty subfamilies of demands for
which the disjoint choice fails, choose one minimal by cardinality; this gives
$\mathcal O$.  If the union of the chosen fan centers, candidate carriers, and
witnessing path/window segments is disconnected, one connected component already
contains a subfamily with no disjoint choice, contradicting minimality unless
the component is taken as the obstruction.  Hence $Z(\mathcal O)$ is connected
after passing to that component.
...
$3$.
\item \emph{Window compatibility:} every packed-window incidence in $Z$ is one
of the $15p_{13}+o(n)$ outside incidences counted by
\cref{lem:stub-positive}; same-window and two-window incidences satisfy the
direct-cycle exclusions of
\cref{lem:typeB-direct-fan-window-cycles,lem:typeB-two-window-cycles}.
\item \emph{Replacement obstruction:} every quotient or identification among the
ledger-response coordinates of $Z$ that would remove a carrier conflict is
either target-defective, target-complete with a smaller proper representative
and therefore forbidden by \cref{cor:uncompressible}, or valid only after
adjoining a larger support and therefore routed to
\cref{lem:proper-smearing,lem:no-silent-global-smearing}.  If none of these
alternatives occurs, the coordinate remains part of the minimal overlap
obstruction.
\item \emph{Minimal overlap:} every proper connected sub-obstruction of
$\mathcal O$ has a refined disjoint ledger.
\end{enumerate}
\end{lemma}
...
For (c), each packed-window incidence is by definition an edge between
$R=G-W$ and the packed window union $W$.  Such edges are exactly the incidence
supply counted in \cref{lem:stub-positive}.  If same-window or two-window labels
violated the displayed arithmetic exclusions, the corresponding direct-cycle
lemma would produce a power-of-two cycle meeting $Z$, contradicting (a).

For (d), regard the candidate ledger entries and their carriers as local
target-response coordinates of the boundaried support $Z$.  If an attempted
identification of two such coordinates is distinguished by some outside
context, it is target-defective by \cref{lem:context-universality}.  If the
identification is target-complete and supplies a smaller proper boundaried
representative with the same boundary degree profile, it is a target-complete
compression and is forbidden by \cref{cor:uncompressible}.  If the
identification becomes valid only after adjoining a larger support, the proper
case is forbidden by \cref{lem:proper-smearing}, and the whole-graph case is
forbidden by \cref{lem:no-silent-global-smearing}.  When none of these
alternatives applies, no quotient, compression, or delocalization removes the
carrier conflict, so the coordinate stays in the overlap obstruction.  These are
exactly the replacement and delocalization alternatives already used in the Type
A saturated exits and in the Type B fan-safe relation.
...
sub-obstruction has a refined disjoint ledger.
\end{proof}
...
overlap obstruction, then B2 holds there; together with the hybrid B1 ledger of
\cref{lem:typeB-hybrid-B1}, \cref{prop:typeB-bridge-reduction} closes the
Type B support.
\end{proposition}
...
ledger.  This is exactly B2.  With the local B1 entries and B2 available,
\cref{prop:typeB-bridge-reduction} gives
\[
\defp(X)-\sigma(X)\ge\frac14|V(X)|
\]
for every connected admissible Type B support outside the route-8 residual.
\end{proof}
...
\node[good,text width=2.65cm] (close) at (5.95,-0.25) {B1 plus B2\\closes the Type B\\support};
...
\cref{lem:typeB-hybrid-B1}; the remaining bridge assertion is the global
disjointness statement B2 used in \cref{prop:typeB-bridge-reduction}.
...
part of B2; clause (d) is the maximal support-assignment part.
\begin{enumerate}[label=(\alph*), leftmargin=2.2em]
...
\begin{lemma}[Post-ledger Type A core hygiene]\label[lemma]{lem:typeB-postledger-core-hygiene}
...
profile is recorded.  In particular, \(Z\) is \(P_{13}\)-free,
contextually dyadic-safe, hereditarily target-uncompressible, and has supplied
deficiency charged to the unused ordinary deficiency reserve and the
window-stub reserve.
\end{lemma}

\begin{proof}
The remaining non-window core is an induced subgraph of the original remainder
core after deleting a declared family of ledger carriers.  Hence every connected
component \(Z\) is \(P_{13}\)-free and contextually dyadic-safe by heredity.
The target-uncompressibility condition is hereditary by \cref{cor:uncompressible}:
a target-complete compression of \(Z\) would extend, with the recorded boundary
profile, to a target-complete compression of the original support.

Deleting ledger carriers can create new deficient boundary vertices in \(Z\).
Each such deficit is caused by one of the removed carrier incidences.  The
carrier was either part of a fan entry already paid in the Type B ledger, an
ordinary deficiency-reserve unit of \cref{def:typeB-ledger-carriers}, or a
packed-window incidence charged by \cref{cor:stub-boundary-supply}.  The
remaining boundary profile records the unused part of those reserves, so the
supplied-deficiency hypothesis in \cref{def:admissible} transfers to each
component.  Since components are treated separately, no connectivity assumption
is inherited across a deleted carrier.  Therefore every connected component is
an admissible Type A support with its own recorded boundary data.
\end{proof}

\begin{proposition}[Type B reduction under the refined ledger]\label[proposition]{prop:typeB-bridge-reduction}
...
\cref{lem:typeB-postledger-core-hygiene} to its connected components.  Since
the route-8 residual is excluded, the Type A saturated exits are closed or
handed to the already processed Type B family, and
\cref{lem:typeA-unsaturated-discharge} gives nonnegative charge on each
remaining component.  The certificate-closed fans, the bridge-paid
positive-deficit fans, and the residual components have nonnegative total
charge in the refined augmented ledger.  Clause B2(c) says that these entries
are mutually disjoint and refine $\widehat{\Ch}_B(X)$, so
\[
\widehat{\Ch}_B(X)\ge0.
\]
The identity \textup{(B-ledger)} in \cref{def:typeB-assigned-ledger} gives
\[
\No(X)
=
\widehat{\Ch}_B(X)+\frac14|H_X|
\ge0.
\]
Since $\No(X)=\defp(X)-\sigma(X)-\frac14|V(X)|$, this is exactly
\[
\defp(X)-\sigma(X)\ge\frac14|V(X)|.
\]
\end{proof}
...
Type B fan-window residual, then
\[
\defp(X)-\sigma(X)\ge\frac14|V(X)| ,
\]
so that $\No(X)\ge0$.  Consequently, a Type B support with $\No(X)<0$ and with
no route-8 or positive-deficit fan-window residual is a Type B bridge residual:
it witnesses failure of B2.
\end{lemma}
...
\sigma(X)=\sum_{h\in H_X}(d_G(h)-3).
\]
It is enough to prove $\widehat{\Ch}_B(X)\ge0$, because then the identity
\textup{(B-ledger)} gives $\No(X)\ge0$.
...
decompose \(X_0\) into connected admissible Type A components.  If one of
these components contained a saturated receiver, then
\cref{lem:typeA-saturated-handoff} would close one of exits (1)--(3), (5), or
(6), peel an exit-(4) target-defective routed load, create a decorated Type B
handoff, or create the route-8 residual profile.  The closed exits contradict
the standing target, replacement, or delocalization invariants, while an
exit-(4) peel leaves through the target-defect branch already excluded from the
post-ledger core.
The decorated handoff is excluded by B2(d), including the clause that every
external handoff must already be covered by the grouped envelope step.  The
route-8 residual is excluded by the hypothesis of this
lemma.  Hence every receiver in every component of \(X_0\) is unsaturated, and
\cref{lem:typeA-unsaturated-discharge} gives nonnegative total charge on those
components.

The fan ledger entries have nonnegative augmented charge by Step~1 and B2(a),
and the remaining core has nonnegative charge by the unsaturated discharging
lemma.  Clause B2(c) says that these entries are a disjoint refinement of
$\widehat{\Ch}_B(X)$, so summing the pieces gives
\[
\widehat{\Ch}_B(X)\ge0 .
\]
By \textup{(B-ledger)}, $\No(X)\ge0$, equivalently
$\defp(X)-\sigma(X)\ge\frac14|V(X)|$.
\end{proof}
...
\(\No_-(X)\le8m_4(X)\).  Summed in the near-cubic spine, allowing one
canonical use and one grouped-envelope use of a center surplus,
\(M_B\le16\sigma(G)=O(\sqrt n)=o(|R|)\).
&
...
left after deleting the grouped fan envelopes of \(\mathfrak C\) contains no
admissible route-8 Type A residual profile.  Then
\[
\No_-(X)\le 8\sum_{h\in H_{\mathfrak C}}(d_G(h)-3).
\]
\end{lemma}
...
over \(h\in H_{\mathfrak C}\) gives the displayed center-token bound.  Grouping
...
admissible Type A components, and
\cref{lem:typeA-saturated-handoff,lem:typeA-unsaturated-discharge} gives
nonnegative charge on those components.  Therefore only the displayed grouped
fan-envelope negative part can remain unpaid, giving the asserted bound.
\end{proof}
...
\[
\No(X)\ge
-D_A(\mathcal A_X)-8\sum_{h\in H_{\mathfrak C}}(d_G(h)-3).
\]
\end{lemma}
...
\cref{lem:typeA-exclusion} gives nonnegative net charge on it.
...
\(-D_A(\mathcal A_X)\).  Combining this with the center-envelope estimate gives
...
admissible route-8 Type A residual profile.  Then
\[
\No_-(X)\le 8\sum_{h\in H_X}(d_G(h)-3).
\]
\end{lemma}
...
\cref{lem:typeA-unsaturated-discharge} give nonnegative net charge on each
component.  Combining this nonnegative core contribution with (2) and the identity
\textup{(B-ledger)} gives
\[
\No(X)
\ge
-8\sum_{h\in H_X}(d_G(h)-3)+\frac14|H_X|.
\]
Therefore
\[
\No_-(X)\le8\sum_{h\in H_X}(d_G(h)-3),
\]
as claimed.
\end{proof}
...
route-8 residual supports contained in the remaining non-window core.  Then
\[
\No(X)\ge
-D_A(\mathcal A_X)-8\sum_{h\in H_X}(d_G(h)-3).
\]
\end{lemma}
...
Every other Type A component has no route-8 residual and no decorated Type B
handoff, so \cref{lem:typeA-exclusion} gives nonnegative net charge on it.
...
=-\left(\frac14|V(Y)|-\defp(Y)\right).
\]
Summing over $Y\in\mathcal A_X$ gives total core contribution
$-D_A(\mathcal A_X)$.  Combining this core contribution with the fan-envelope
bound and the identity \textup{(B-ledger)} gives the displayed inequality.
\end{proof}
...
supports whose non-window cores contain no admissible route-8 Type A residual
profile.  Then
\[
M_B(\mathcal X_B)\le 8\,S_B(\mathcal X_B)
\le 16\,\sigma(G)
=O(\sqrt n)
=o(|R|).
\]
...
\(X\in\mathcal X_B\) gives \(M_B(\mathcal X_B)\le8S_B(\mathcal X_B)\).  The
...
\cref{prop:typeB-bridge-reduction} gives $\No(X)\ge0$; in the
certificate-closed specialization this is exactly \cref{lem:typeB-exclusion}.
...
\ \ge\ -\tilde D_A-8S_B(\mathcal X_B)-o(|R|)
\ \ge\ -\tilde D_A-o(|R|),
\]
...
\tilde N=|\tilde\Xi|.
\]
An entry is a \emph{route-8 entry} if $B_u$ is target-complete-minimal
(\cref{def:typeA-trace-basin}); otherwise it is a \emph{target-defect entry},
and one of the four failure alternatives of \cref{def:typeA-trace-basin} holds
for $B_u$.
\end{definition}
...
\[
\alpha_{\mathcal X}(\xi)=|\mathcal C_{\rm ess}(\xi)|\ \ge\ 2 .
\]
\end{lemma}

\begin{proof}
For a route-8 entry this is \cref{lem:typeA-one-terminal-collapse}.  Let $\xi$ be
a target-defect entry and put $\mathcal C=\mathcal C_{\rm ess}(\xi)$.  Suppose
...
an entry $\xi$ with $\pi_{\mathcal X}(\xi)\le2$.
\end{proposition}

\begin{proof}
Suppose not; then every $\xi\in\tilde\Xi$ has $\pi_{\mathcal X}(\xi)\ge3$.  By
...
$B_u$ is target-complete-minimal and $\pi_{\mathcal X}(\xi)\le2$, the declared
deletion witnesses make \((\tilde{\mathcal X},\xi)\) a terminal two-carrier
route-8 obstruction, which is excluded by
\cref{thm:typeA-two-carrier-nogo}.  The new accounting below treats only the
...
consists of three private essential carriers of \(\xi\).  Thus no surviving
route-8 entry is placed in \(\Xi_2\) or \(\Xi_{\rm res}\);
...
route-8 entry in \(\Xi_3\) with three private essential carriers.  A
...
level.  Since surviving route-8 entries never lie in \(\Xi_2\cup\Xi_{\rm res}\),
...
For every \(s\ge0\),
\[
D_s=\{2^q-s-2:q\ge1,\ 0\le2^q-s-2\le12\}.
\]
In particular,
\[
\begin{array}{c|l}
...
\(s+2+d=2^q\) for some \(q\ge1\).  This is equivalent to
\[
d=2^q-s-2
\]
...
\]
then \(\tilde\Xi^{P_4}\) contains a two-carrier entry.
\end{lemma}
...
that no entry of \(\tilde\Xi^{P_4}\) is two-carrier.  Then every remaining
entry has at least three private essential carriers.  These private carriers
...
of \(\pi_{\mathcal X}\) in \cref{def:typeA-route8-carriers}.  Deleting peeled
...
\[
\sum_i\No(X_i)\ge -D_A(\mathcal X_A^0)-8S_B,
\]
...
\node[good,text width=1.95cm] (bclosed) at (1.20,-0.85) {B1 plus B2\\gives\\\(\No(X)\ge0\)};
...
{[74]} \texttt{fanthm} (Type B, $\sigma>0$) & \cref{lem:typeB-exclusion} and \cref{prop:typeB-bridge-sublinear} via \cref{def:marked-typeB-fan}, \cref{def:typeB-window-incidence-profile}, \cref{def:typeB-hybrid-incidence}, \cref{def:closed-fan-window-pair}, \cref{def:direct-cycle-free-closed-pair}, \cref{lem:typeB-direct-fan-window-cycles}, \cref{lem:typeB-two-window-cycles}, \cref{lem:typeB-multiclosed-budget}, \cref{lem:typeB-hybrid-incidence-budget}, \cref{lem:typeB-hybrid-B1}, \cref{def:typeB-ledger-carriers}, \cref{def:typeB-candidate-ledger}, \cref{def:typeB-overlap-obstruction}, \cref{lem:typeB-bridge-to-overlap}, \cref{lem:typeB-global-local-reflection}, \cref{prop:typeB-global-local-bridge}, \cref{def:typeB-bridge-statements}, \cref{prop:typeB-bridge-reduction}, \cref{def:typeB-multiclosed-residual}, \cref{def:typeB-residual-mass}, \cref{lem:decorated-envelope-with-route8-core}, \cref{lem:typeB-bridge-deficit-bound}, \cref{lem:typeB-bridge-with-route8-core}, \cref{rem:fan-finite}, and \cref{lem:fan-certificate} & outside route 8, outside fan-certificate residual centers, and outside B2 failure, every certificate-marked fan has $d_G(h)\le8$, every nonpositive-deficit marked fan has nonnegative closed-neighbourhood charge, every positive-deficit fan has the local hybrid B1 incidence ledger, and B2 supplies the disjoint ledger needed to sum the fans; in the near-cubic spine, residual centers and B2 failures satisfy $M_B\le16\sigma(G)=o(|R|)$ after any route-8 non-window core is extracted into $D_A$ and grouped-envelope centers are counted at most twice & certificate-closed iff $D_B\le0$; every positive-deficit residual has $D_B=c-\tfrac{11-k}{4}$ paid by half-credit on the $2c$ disjoint non-$h$ incidences, split into window credit $I_W/2$ and non-window demand $D_N$; each bridge residual is charged by at most eight times its assigned surplus plus the extracted route-8 Type A deficit & \textbf{Structural reduction to B2 plus fan-mass closure.} Direct fan-window and two-window cycles close by explicit path construction; local B1 is the hybrid incidence budget; every disjoint-carrier B2 failure is represented by a minimal overlap obstruction carrying contextual dyadic safety, window-stub capacity, replacement/delocalization routing, and a sublinear fan-mass budget in the near-cubic spine after route-8 cores are charged to $D_A$; fan-certificate residual centers and grouped envelopes enter this fan-mass bound directly. & In the near-cubic spine, a surviving Type B bridge obstruction has only sublinear bridge mass outside the Type A pressure/route-8 ledger, so it cannot carry the linear large-budget obstruction without entering node {[123]}. \\
...
\bibitem{HSS2024}
A. S. Hegde, R. Sandeep, and S. Shashank,
\emph{Power of two cycles in $P_{13}$-free graphs},
arXiv:2410.22842, 2024/2025.
\url{https://arxiv.org/pdf/2410.22842}
