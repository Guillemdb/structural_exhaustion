...
namespace StructuralExhaustion.CT6.Nodes.Scope
...
structure Plan (F : Framework) (input : Input F) where
  decide : Contract F input

def run {F : Framework} {input : Input F}
    (_entry : Input F) (plan : Plan F input) : Contract F input := plan.decide

end StructuralExhaustion.CT6.Nodes.Scope
