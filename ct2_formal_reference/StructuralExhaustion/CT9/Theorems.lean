...
theorem outcome_exhaustive {F : Framework} {input : Input F}
    ...
    result.terminal = .ct4 ∨ result.terminal = .c1 ∨
    result.terminal = .ct7 ∨ result.terminal = .ct8 := by
  cases result with
  | mk _ _ outcome =>
      cases outcome with
      ...
      | c1 _ => exact Or.inr (Or.inr (Or.inr (Or.inl rfl)))
      | ct7 _ _ => exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl))))
      | ct8 _ _ => exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr rfl))))
