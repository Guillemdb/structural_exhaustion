...
def run {F : Framework} {input : Input F} (_entry : Input F)
    (plan : Plan F input) : Contract F input := plan.decide
end StructuralExhaustion.CT9.Nodes.Scope
