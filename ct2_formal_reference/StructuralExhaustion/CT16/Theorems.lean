...
theorem verified {F : Framework} {input : Input F} {port : Port F input}
  ...
  cases o with
  | ct3 _ h => exact h | scope c => exact c.unavailable
  | c2 c => exact c.closes | ct10 _ h => exact h
end Outcome
...
theorem outcome_exhaustive {F : Framework} {input : Input F} {port : Port F input}
    ...
    r.terminal = .ct3 ∨ r.terminal = .scope ∨ r.terminal = .c2 ∨ r.terminal = .ct10 := by
  cases r with
  | mk _ _ o => cases o with
    | ct3 _ _ => exact Or.inl rfl
    | scope _ => exact Or.inr (Or.inl rfl)
    | c2 _ => exact Or.inr (Or.inr (Or.inl rfl))
    | ct10 _ _ => exact Or.inr (Or.inr (Or.inr rfl))
