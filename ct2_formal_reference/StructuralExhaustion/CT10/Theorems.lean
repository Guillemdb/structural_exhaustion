...
theorem verified {F : Framework} {input : Input F} {port : Port F input}
  ...
  | ct15 _ accepted => exact accepted
end Outcome
...
theorem outcome_exhaustive {F : Framework} {input : Input F}
    ...
    result.terminal = .scope ∨ result.terminal = .c5 ∨
    result.terminal = .ct3 ∨ result.terminal = .ct7 ∨
    result.terminal = .ct15 := by
  cases result with
  | mk _ _ outcome =>
      cases outcome with
      | scope _ => exact Or.inl rfl
      | c5 _ => exact Or.inr (Or.inl rfl)
      | ct3 _ _ => exact Or.inr (Or.inr (Or.inl rfl))
      | ct7 _ _ => exact Or.inr (Or.inr (Or.inr (Or.inl rfl)))
      | ct15 _ _ => exact Or.inr (Or.inr (Or.inr (Or.inr rfl)))
theorem ct3_adapter_exact {F : Framework} {input : Input F}
    (payload : CT3Payload F input) : payload.toInput =
      match payload with
      | .direct p => p.downstream
      | .promoted p => p.downstream := by cases payload <;> rfl
