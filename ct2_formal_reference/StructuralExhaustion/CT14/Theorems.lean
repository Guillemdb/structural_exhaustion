...
theorem verified {F : Framework} {input : Input F} {port : Port F input}
    (result : ExecutionResult F input port) : OutcomeClaim result.outcome := result.outcome.verified
...
theorem outcome_exhaustive {F : Framework} {input : Input F} {port : Port F input}
    (result : ExecutionResult F input port) :
    result.terminal = .scope ∨ result.terminal = .ct9 ∨
    result.terminal = .ct10 ∨ result.terminal = .c4 := by
  cases result with
  | mk _ _ outcome => cases outcome with
    | scope _ => exact Or.inl rfl
    | ct9 _ _ => exact Or.inr (Or.inl rfl)
    | ct10 _ _ => exact Or.inr (Or.inr (Or.inl rfl))
    | c4 _ => exact Or.inr (Or.inr (Or.inr rfl))
