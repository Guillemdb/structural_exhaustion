...
This remark records the manuscript route, not unconditional Lean coverage.
The Lean-owned proved frontier currently ends at node~[20].  Nodes~[21]--[60]
are partial: their local transformations compile, but proved producers are
still missing at nodes~[21], [23], [35], [48], [52], [54], [56], and [57].
Within that partial chain, nodes~[58]--[60] add only the
paper's exact net-charge definition, exhaustive sign split, and local
nonnegative-branch contradiction; their kernel-checked implementations add
no further assumption or diagram edge.
...
15 & [21] & Curvature enumeration & $543958,432672,111286$ and $c_\Omega$ & finite computation; current Lean producer conditional on the typed node-[21] carrier & \cref{lem:curv-enum} \\
16 & [21], [145] & Weighted live/cold window interface & in packing order, extend the recoverable joint hot aggregate by one complete safe/flat package at \(\log_2n-O(1)\) separated scales, or retain the identical window with failure of that exact extension & bounded dependent hot product or cold geometric route & \cref{lem:p13-window-package,def:cold-window-ledger}; \cref{app:curv-code} \\
16a--r & [153] & Unnumbered local support for first-failure extraction & same-window attachment, return normalization, local component schedules, D1--D7 projections, and bounded CT10/CT8 obligations are retained as internal producers and residuals of node [153] & no auxiliary diagram vertices or edges; each computation scans only its actual finite support & \cref{rem:p13-weighted-live-cold-handoff,rem:p13-component-d4d7-full-semantics,rem:p13-long-prefix-full-semantics} \\
...
18 & [145]--[157] & Hot/cold window branch & if \(\theta<1/78\), route 8 closes; otherwise hot failure forces \(C\ge(\theta-\theta_{\rm win})n-o(n)\) and \(b(\mathfrak S_{\rm cold})\ge13C-o(n)\); node [153] reads prefixes stage-major, with a literal attachment scan for F1 followed at the same stage by F2, F3 and F4, and only exhaustive F5 enters [154] & the graph-owned F2--F5 producer and the resulting germ bound \(N_{\rm germ}\ge13C/D_{\rm cold}-o(n)\) remain open & \cref{def:cold-window-ledger,def:surviving-cold-branch,lem:hot-failure-cold-mass,def:cold-skeleton-excess,lem:cold-window-stub-excess,lem:cold-short-self-return-filter,def:cold-bounded-germ,def:cold-corridor-first-failure,lem:cold-corridor-first-failure,lem:cold-germ-extraction,lem:cold-bounded-germ-trichotomy,def:cold-same-interface-table,lem:cold-same-interface-table,lem:cold-increment-arithmetic,thm:cold-branch-quantitative-closure} \\
...
31 & [49] & Constrained remainder-graph entropy & $\eta(R)=\log_2|\mathcal G(R)|/|R|$ for the four inherited remainder constraints & separates high-/low-entropy regimes & \cref{def:remainder-entropy} \\
32 & [49], [50] & Dominant local type & the low-entropy branch plus a separately proved subexponential radius-$2$ type coordinate and relabelling closure gives a dominant rooted type & otherwise too many labelled type vectors & \cref{lem:dominant-type} \\
33 & [51] & High remainder bit contribution & $\eta(R)\ge(1/10)\log_2n$ gives $(|R|/10)\log_2n\le\log_2|\mathcal G(R)|$ & symbolic real arithmetic & \cref{lem:high-remainder-bits} \\
...
51 & [140], [142]--[144] & Geometric bottleneck classification and semantic frontier & a large same-token fibre gives a homogeneous matching or star; internal node-[144] support computes the coarse collision, classifies the rooted germs, and exposes the local separator split & the finite support stages are verified locally; the sparse-exit, CT3, Type B, or fixed-cap consumer remains open inside [144] & \cref{lem:same-token-semantic-bottleneck-classification,lem:same-token-local-semantic-consumer,lem:same-token-bottleneck-routing} \\
\bottomrule
\end{longtable}
}
...
\cref{prop:exact-25-role-coupled-decision} & ENT\slash SURP & [137]--[139], [141] \texttt{(exact coupled decision)} & conditional Lean support for nodes [126], [130]--[136]; \cref{lem:capacity-token-supply,lem:total-pair-token-route-no-overcount} & Executes CT9 on the literal complete pair schedule and the exact 25-role alphabet. & A positive excess returns an actual overloaded fibre and its unique token-class route; the other side proves the explicit node-[138] quadratic surplus bound. \\
...
\cref{lem:same-token-coarse-bottleneck-classification} & ENT\slash SURP & [144] \texttt{(coarse collision)} & conditional Lean support for nodes [140], [142], [143] & Executes the exact 49-item/48-code scan and retains two distinct equal-code pairs, literal attachment maps, and two canonical rooted germs. & Routes the unchanged residual to the local finite-classifier interface for [144] without asserting a semantic conclusion. \\
\cref{lem:same-token-semantic-bottleneck-classification,rem:same-token-semantic-bottleneck-classification-scope} & ENT\slash SURP & [144] \texttt{(finite classifier)} & the conditional coarse node-[144] trigger & Scans the literal \(78p_{13}\) attachment coordinates, returns the first mismatch or full alignment, and then records one of four exact rooted-germ shapes in an exhaustive CT10 table. & Routes the unchanged five-way residual to [144]; it asserts no sparse-exit, CT3, Type B, fixed-cap, or spine conclusion. \\
\cref{lem:same-token-bottleneck-routing}, \cref{thm:homogeneous-overload-geometric-closure} & ENT\slash SURP & [144] \texttt{(semantic bottleneck)} & inv 4, 8, 14, 15; node [144]; sparse surplus exits and Type B routing where applicable & The homogeneous bottleneck realizes a sparse surplus exit, produces Type B fan data, or falls under fixed homogeneous caps. & Open consumer: each finite-classifier branch still requires its complete certificate. \\
...
\cref{prop:nonnear-cubic-sharp-overload-routing} & ENT\slash SURP & [144] \texttt{(semantic bottleneck)} & inv 14, 15; \cref{cor:homogeneous-same-token-caps-close} & Combines the sparse-pressure discharge with the cap-to-spine route. & Remains downstream of the open semantic consumer. \\
...
\cref{lem:p13-window-package,rem:p13-weighted-live-cold-handoff} & ENT\slash EXC & [21] $\to$ [145] \texttt{(weighted split)} & exact packing order, current aggregate, the 91 safe/flat rows, graph-owned local interpretation, recoverable joint choices, and bounded injective spine code & Accepts one compatible extension or retains the identical window, prior aggregate, and extension failure; the sequential cold list executes the same-window surplus/corridor route. & Joint capacity, exact partition, and [151]--[153] provenance are verified; the normalized hot arithmetic and terminal cold closure remain open before [22]--[24]. \\
\cref{rem:p13-weighted-live-cold-handoff,rem:p13-component-d4d7-full-semantics,rem:p13-long-prefix-full-semantics} & EXC & [153] \texttt{(internal support)} & exact cold-window residuals, literal local graph schedules, and predecessor provenance & Retains the finite same-window, short-return, long-prefix, component, and D4--D7 support stages as internal producers for original node [153]. & No auxiliary diagram node or edge is introduced; open semantic consumers remain typed residuals inside [153]. \\
\cref{def:cold-window-ledger,def:surviving-cold-branch,lem:hot-failure-cold-mass,def:cold-skeleton-excess,lem:cold-window-stub-excess,lem:cold-short-self-return-filter,def:cold-bounded-germ,def:cold-corridor-first-failure,lem:cold-corridor-first-failure,lem:cold-germ-extraction,lem:cold-bounded-germ-trichotomy,def:cold-same-interface-table,lem:cold-same-interface-table,lem:cold-increment-arithmetic,thm:cold-branch-quantitative-closure} & ENT\slash EXC & [145]--[157] \texttt{(cold)} & inv 1, 8, 14, 15, 20, 35, 38; \cref{def:near-cubic-spine}; \cref{lem:p13-window-package}; \cref{thm:large-budget-route8-only} & The cold continuation at [153] is stage-major: literal attachment F1, then F2/F3/F4 at the same prefix, with only exhaustive F5 entering [154].  The F1 constructor is verified; the graph-owned F2--F5 producer and the germ bound remain open. & Node [153] is not green and supplies no terminal cold-window closure yet. \\
...
\cref{prop:two-budget} & 2BUD & [52] \texttt{(windowacct)} & inv 20, 27, 29 & Two-budget routing: high entropy closes by window count; repetitive low entropy gives a rank input; nonrepetitive low entropy routes forward. & Prevents entropy overcounting and preserves the net-charge route. \\
\cref{lem:low-entropy-forced-cost-fit} & 2BUD & [53] \texttt{(smallbudget)} & node [48] realized ledger; node [49] exact state count; node [50] strict low constructor & The exact forced curvature power fits strictly inside the flat power times the low-entropy allowance, so the printed small-budget edge is impossible. & The complementary no constructor is node [55]; the strict-quarter cap belongs only to node [56]. \\
...
\cref{prop:typeA-route8-carrier-reduction} & EXC & [117]--[123] & inv 14, 23, 24; \cref{cor:stub-boundary-supply}, \cref{lem:typeA-route8-burden}, \cref{lem:typeA-one-terminal-collapse} & If no two-carrier entry exists, three private carriers per basin violate the boundary-incidence budget; hence a two-carrier entry exists. & The CT4 private-carrier injection and exact inequality (3|\Xi|\le |\mathcal C_{\rm priv}|) are verified.  The graph-owned [107]-no to [109]--[114] trace-basin and carrier-core producer remains the first open input. \\
...
\cref{lem:typeB-bridge-with-route8-core} & EXC & [84] \texttt{(global fanmass)} & inv 4, 14, 16, 24, 35; \cref{def:typeB-residual-mass} & Extracts a route-8 non-window core from Type B bridge residual data into \(D_A\). & Keeps Type B bridge mass separate from Type A route-8 deficit. \\
\cref{lem:typeB-processed-boundary-bound,lem:typeB-bridge-deficit-bound}, \cref{prop:typeB-bridge-sublinear} & EXC & [84] \texttt{(global fanmass)} & inv 4, 14, 16, 24, 35; \cref{def:near-cubic-spine}; \cref{lem:typeB-hybrid-B1}, \cref{def:typeB-bridge-statements} & Charges residual centers, processed-envelope boundary loss, and B2 failures to high-degree fan-center surplus and obtains \(M_B\le416\sigma(G)=o(|R|)\). & Open producer contract: construct the canonical ordinary/grouped family and both coefficient-\(208\) incidence bounds before this consequence is available. \\
...
entry.  Its exact residual is passed to the finite classifier at node [144].
...
Node [144] proves only the preceding five-way local classification.  It does
...
near-cubic spine.  Node [144] performs only the verified local separator
split below; the stronger branchwise implications remain open at node [144].
\end{remark}

\begin{lemma}[Local semantic-bottleneck separator split]\label[lemma]{lem:same-token-local-semantic-consumer}
Consume node [144]'s exact five-way result.  A mismatch or either prefix
...
\begin{proof}
Inspect the constructor returned at node [144].  The first three constructors
...
Consume node [144]'s exact local frontier.  Mismatch and aligned-prefix leaves
...
Consume node [144] exactly.  Each cubic leaf projects its literal cubic-star
...
Consume node [144] exactly and retain its literal payload.  Tag mismatch,
...
Consume node [144] exactly.  Mismatch and prefix leaves retain their evidence.
...
Consume node [144] exactly and retain its pending obligation unchanged.  On a
...
The finite curvature computation gives the local safe and flat counts.  The
paper uses them as conditional fibre costs, not as coordinates of a Boolean
cube.  The exact live/cold interface is recorded next.

\begin{remark}[Weighted multi-scale \(P_{13}\)-window package]\label[remark]{lem:p13-window-package}
Fix one selected window.  At the separated dyadic scales, order the declared
barrier tests and write
\[
\mathcal S_0\supseteq\mathcal S_1\supseteq\cdots\supseteq\mathcal S_r
\]
for the successive fibres of graph-owned connector states.  If the \(j\)-th
test has barrier index \(i(j)\), a live package carries the local inequality
\[
W_{i(j)}|\mathcal S_{j+1}|
\le F_{i(j)}|\mathcal S_j|,
\]
where \(W_i\) and \(F_i\) are the verified safe and flat counts.  It also
carries a nonempty final fibre, an injective graph-owned connector code, and
the occurrence of every one of the \(91\) barriers at
\(\log_2n-O(1)\) separated scales.  Multiplying the displayed local
inequalities gives the full variable-factor product cost and hence the
constant
\[
c_{13}=\sum_i\log_2(W_i/F_i)=118.10858100609198\ldots .
\]

A live package is not tested independently of the packages already retained.
Process the fixed packing order while carrying one jointly compatible hot
aggregate.  At a window \(P\), accept only an extension which adjoins the
complete local package to that aggregate, gives recoverable simultaneous
choices for every retained owner, preserves their graph responses, and has an
injective code in the fixed spine family.  If no such extension exists, keep
the previous aggregate and retain that identical \(P\) as a cold residual.
Thus dependence is the cold outcome prescribed by nodes [145]--[157], not a
third branch, and no coordinatewise activity is promoted to simultaneous
Boolean realization.

Conditional on the exact typed node-[21] output, the Lean interface verifies
the variable-factor telescope, positive scale
multiplicity with bounded scale loss, graph-semantic connector tests, and the
packing-order compatible-extension ledger.  Its final aggregate carries a
finite dependent joint-state family, a left inverse recovering every retained
local choice, response commutation for each recovered owner, and an injective
code bounded by the fixed spine capacity.  Rejection records the exact prior
aggregate and the failure of its extension type.  The accepted and rejected
lists partition the original packing exactly.  On the negative side, this
sequential rejection list is filtered by
the graph-owned ambient-cubic predicate without replacing it by the ambient
packing: the cubic and non-cubic sublists partition that same list.  Every
retained cubic cold window has exactly fifteen literal external stubs, its
first two stubs are the transit stubs, and the remaining thirteen form an
exact proof-carrying branch-excess schedule whose entries retain their cold
window of origin.  Each entry is passed unchanged to the outside-component
corridor runner.  This formalizes the local handoffs through nodes [151] and
[152].  Moreover, the cold list has no repeated selected window.  Its
non-cubic sublist therefore injects into the packing's non-cubic windows, and
the local surplus payment gives
\[
 |\mathcal P_{\rm cold}^{\rm noncub}|\le \sigma(G),
 \qquad
 13|\mathcal P_{\rm cold}|
 \le |\mathcal E_{\rm br}|+13\sigma(G).
\]
These are exact finite inequalities; no asymptotic estimate or ambient-window
substitution is used.

The typed node-[21] output retains the exact inequality
\(\sigma(G)^2\le K n\), the discarded loss also satisfies
\(|\mathcal P_{\rm cold}^{\rm noncub}|^2\le K n\).  This is the finite
square-root form of the paper's \(o(n)\) assertion and uses only that typed
predecessor; it is not unconditional node-[21] evidence.

The remaining hot-side obligation is the exact arithmetic passage from the
per-package weighted rate bounds to the dependent product cardinality, and
from the fixed spine-state capacity to the printed normalized hot bound.  On
the cold side, the literal stage-major F1 constructor is verified; node [153]
still requires the graph-owned F2--F5 semantic producer described in
\cref{rem:cold-first-failure-producer}.  Until those obligations are
discharged, the density conclusion is not used as a proved input.
\end{remark}
...
Node [153] is a pointwise thirteen-coordinate adjacency scan.  It is distinct
...
Fix the node-[153] cold residual on one selected induced \(P_{13}\) window.
...
If node [153] returns its dyadic constructor, then the constructor's exact
...
Node [153] is pointwise and structural.  Its window-surplus and corridor-high
...
bounded germ.  Thus the computed dyadic constructor has conditional Lean
evidence for node [155], but node [155] is not green in the current Lean-owned
diagram status.  No cold-mass, density, bounded-overlap, or nodes [153]--[157]
aggregate conclusion follows.
\end{remark}

\begin{lemma}[D1--D3 base-scale split]\label[lemma]{lem:p13-d1d3-base-scale-split}
Suppose the computed node-[153] output is its quiet constructor, with its exact
...
Node [153] does not assert that an empty local-coordinate alphabet captures
...
in two internal stages of node [153], without adding those missing semantics.  This
auxiliary split is independent of the weighted live/cold handoff in
\cref{rem:p13-weighted-live-cold-handoff} and supplies no window entropy.
\end{remark}

\begin{lemma}[Short-return third incidence]\label[lemma]{lem:p13-short-third-incidence}
Suppose the computed node-[153] result is its short constructor.  At the root
...
Node [153] inspects only this one root incidence.  Its on-support and outside
outcomes feed the separately implemented internal node-[153] branch consumers, but
not their normalized one-return rejoin [153].  It supplies no cold-skeleton
boundary-response semantics, which remain further downstream at [153].  No
...
Suppose the computed node-[153] result is its on-support constructor, and let
...
\begin{proof}
The input stores equality with node [153]'s actual on-support result.  Scan the
...
Node [153] closes only the accepted target-length constructor and otherwise
returns one exact strictly shorter deleted-edge return.  It does not normalize
the two node-[153] branches into a common residual, iterate the shortening,
prove termination, reconstruct D4--D7, execute CT3, or derive density.  Its
exact shorter residual is consumed by the conditional proof-level rejoin [153]
and packed-support transition [153]; its two computed outcomes feed the
separately implemented internal node-[153] branch consumers.
\end{remark}

\begin{lemma}[Outside-return cubic boundary star]\label[lemma]{lem:p13-outside-boundary-star}
Suppose the computed node-[153] result is its outside constructor.  Orient the
...
zero additional primitive checks beyond node [153].
\end{lemma}

\begin{proof}
The input includes equality with the actual node-[153] runner, not merely an
...
return step and restored endpoint.  Node [153] has already proved that the
...
Node [153] certifies only this one oriented support crossing and the exact
...
consumed by the conditional proof-level rejoin [153] and packed-support transition
[153]; its two computed outcomes feed the separately implemented downstream
internal node-[153] branch consumers.
\end{remark}

\begin{lemma}[Normalized one-return boundary]\label[lemma]{lem:p13-normalized-return-boundary}
Fix the common computed node-[153] short residual.  Suppose the input is either
the exact rejected-chord computation from node [153] or the exact outside
computation from node [153].  There is a selected deleted-edge return
...
rejected-chord branch select node [153]'s exact shorter return.  Its old first
...
resolver.  On the outside branch keep node [153]'s original return and selected
...
Node [153] is a proof-level normalization of one return only.  It does not
itself scan packed support.  Its exact computed output is consumed by node
[153], whose exact all-inside branch subsequently feeds the conditional
owner-change interface inside node [153].  That internal support stage supplies no owner table or cross-window token,
successor or second stub, component path [153], iteration or termination
...
Let \(\Gamma'\) be the exact computed normalized return from node [153], and
let
\[
...
disjoint selected-window packing bound gives \(13p\le n\), while node [153]
...
not the manuscript's selected cold subfamily, so node [153] proves no cold
aggregate or cold-mass statement.  Its exact all-inside output is consumed by
the conditional owner-change interface inside node [153].  The transition output constructs one exact
stub, endpoint, and component, but no successor, second stub, or path through
that component; conditional node-[153] support computes those objects on the exact
first-transition branch.  There is no
iteration, termination, D4--D7 reconstruction, CT3 execution, or density
conclusion.
\end{remark}

\begin{lemma}[Component boundary schedule and lexicographically first path]\label[lemma]{lem:p13-component-boundary-schedule}
Suppose node [153] returns its exact first-transition constructor, with
...
\item among the simple paths in \(C\) from the outside endpoint of \(b\) to
  that of \(b^+\), the stored corridor is the lexicographically first one in
  the fixed component-vertex order.  Its endpoints are exact, it is simple,
  and its length is strictly below \(|V(C)|\).
\end{enumerate}
...
object use the same computed outside BFS finset.  If \(S\) denotes the sum of
one plus the cardinalities of all explicitly scanned local collections, the
complete visible schedule work is at most \(50S^3\); the retained simple path
has fewer than (|V(C)|) edges.
\end{lemma}

\begin{proof}
Retain equality with node [153]'s actual first-transition run, so the anchor
...
\(b^+\ne b\).  Restricting to the computed component object, the declared-order
BFS proves that the two outside endpoints are connected.  Apply the finite
lexicographic simple-path selector to precisely that component and those two
endpoints.  Its selected path has the stated head and last vertex, is simple,
and no simple endpoint path has a smaller forward vertex-position word.  The
standard simple-path bound gives length strictly below (|V(C)|).
...
scale give \(50S^3\).  The lexicographic selection is confined to this one
supplied finite component; it does not enumerate graphs or outside contexts.
\end{proof}

\begin{remark}[Scope of the component boundary schedule]\label[remark]{rem:p13-component-boundary-schedule-scope}
This internal support stage lies only on the short first-transition branch of
node [153].  Its exact schedule and lexicographically first simple path feed the one-state component
observation internal to that node.  The long-support branch and its open full
semantic consumer are incompatible alternatives; no diagram edge is added
between these internal branches.  This stage proves no D4--D7 label,
...
Let node [153] return its exact component boundary schedule with anchor stub
...
\begin{proof}
Use node [153]'s actual graph input, not a reconstructed component or supplied
...
This internal node-[153] stage produces one state, not a sequence or repetition
theorem.  Its missing-reconstruction residual feeds the cyclic component
D1--D3 ledger split internal to the same node.  Only that split may feed the
open D4--D7 reconstruction or coarse-repeat consumer.  These internal routes
do not enter the mutually exclusive long-branch classifier or its semantic
consumer.  This stage
...
Let \(\mathcal S_C\) be node [153]'s complete duplicate-free incident-stub
schedule, with its stored cyclic successor, and let \(s_C\) be the exact
node-[153] residual.  There is one locally computed D1--D3 row \(s_x\) for
...
\begin{proof}
The ledger input retains the typed node-[153] residual together with equality
to its actual execution.  At the anchor stub, the row constructor returns this
retained state definitionally.  At every other stub it re-anchors node [153]'s
...
This internal node-[153] stage compares only the coarse states of rows actually produced from the
...
residuals feed the conditional finite classifier internal to node [153].  This stage proves no CT3
...
diagram edge to the mutually exclusive long-branch support stages.
\end{remark}

\begin{lemma}[D4--D7 availability and coarse-repeat classifier]\label[lemma]{lem:p13-component-d4d7-classifier}
Consume node [153]'s exact generic result together with its equality to the
...
\begin{proof}
Run the graph-owned classifier on node [153]'s retained result.  Its repeated
...
Node [153] proves availability or exposes one exact missing refinement.  It
...
cycle, or prove target closure.  Node [153] performs only the exact readiness
split below; the semantic implications remain white at node [153].
\end{remark}

\begin{lemma}[Component D4--D7 semantic readiness]\label[lemma]{lem:p13-component-d4d7-semantic-readiness}
Consume node [153]'s exact execution.  The bounded complete-reconstruction
...
\begin{proof}
The source stores equality with the specialized node-[153] run.  Membership of
...
Consume node [153]'s exact result.  Retain each dependent missing-clause marker
...
Consume node [153]'s exact dependent ledgers.  Focus (D4) as the next
...
Consume node [153] exactly.  For each actual dependent cursor, emit the
...
Consume node [153] exactly.  Retain every dependent marker, singleton D4
...
Consume node [153] exactly.  Retain each singleton D4 request, its dependent
...
\begin{lemma}[Graph-owned D4 evaluation after node \texorpdfstring{[153]}{[153]}]
\label[lemma]{lem:p13-component-d4-evaluator}
For every construction residual returned inside node [153], the retained marker
determines the same component input and canonical component path as in the
preceding internal support stages.  On the literal support of that path, let a D4 coordinate
...
node-[153] residual, the requested slot is D4, and the unevaluated tail is
...
This is an internal residual interface of original node [153].
...
Suppose node [153] returns its exact all-inside constructor on the computed
...
\begin{proof}
The all-inside equality is the literal output of node [153], so it supplies
...
manuscript's selected cold subfamily.  Node [153] retains exactly one first
...
Let node [153] return its complete computed first-cross-window package.  Node
[153] returns two tokens $t_L,t_R$, their exact endpoint windows and
...
\item the two oriented contributions have the same literal undirected edge,
  \(\{x_L,x_R\}=\{x_R,x_L\}\).
\end{enumerate}
The residual performs exactly zero additional primitive checks after node [153].
\end{lemma}

\begin{proof}
Apply the reusable cross-window token-pair route to node [153]'s retained
...
Node [153] is the exact terminal residual of the all-inside branch.  It constructs no repeated
...
Suppose the computed node-[153] result is its long constructor.  The identical
...
Node [153] supplies only literal finite positions and their exact inclusion.
...
estimate.  Its exact prefix feeds the verified first-nine classifier [153].
\end{remark}

\begin{lemma}[First-nine observed-label refinement]\label[lemma]{lem:p13-long-prefix-observed-label}
Suppose node [153] is the actual forced-prefix result on the computed long
...
\begin{proof}
The exact node-[153] equality identifies every prefix index with the corridor
...
Node [153] proves only equality of the displayed graph-derived coarse label
...
removal or certified smaller object.  Node [153] performs only the exact
degree refinement below; the stronger obligations remain white at [153].
\end{remark}

\begin{lemma}[Two-occurrence exact degree refinement]\label[lemma]{lem:p13-long-prefix-degree-refinement}
Consume node [153]'s exact collided pair and promoted CT10
...
\begin{proof}
The node-[153] equality fixes both occurrences and their literal vertices.
...
Consume node [153]'s exact degree result and retained CT10
...
Consume node [153] exactly.  An inherited first-nine mismatch passes through
...
Consume node [153] exactly.  Either inherited mismatch passes through with
...
Consume node [153] exactly.  Each inherited mismatch passes through unchanged.
...
Consume node [153] exactly and retain its four possible mismatch leaves and
...
This is a verified internal residual interface of original node [153].  Its
...
\begin{remark}[Formal weighted live/cold handoff]\label[remark]{rem:p13-weighted-live-cold-handoff}
From the accumulated framework ledger at node [21], the formal handoff processes
the exact node-[21] packing in its declared
order.  Starting from the original graph as the empty aggregate, it either
accepts a graph-semantic compatible extension of the current aggregate or
rejects that identical window while leaving the aggregate unchanged.  A
rejection stores the current aggregate and
\(\neg\operatorname{Nonempty}(\operatorname{Extension}(A,P))\); it does not
assert that \(P\) lacks a standalone local package.  The accepted aggregate
stores recoverable simultaneous local choices and a bounded injective spine
code.  The split is proof-level and enumerates no ambient graph, context,
state subset, or Boolean universe.  The pointwise thirteen-bit classifier at
internal stages of node [153] remains auxiliary same-window geometry.
\end{remark}
...
Fix the declared order on a maximal vertex-disjoint induced-\(P_{13}\) packing
\(\mathcal P\).  A packed window is \emph{hot} when its compatible extension
is accepted into the aggregate accumulated from the preceding windows.  It is
\emph{cold} when that extension type is empty at its exact turn.  Rejection
retains the same aggregate and the same window.  The one sequential run gives
the exact partition
\[
...
\]
Only a hot certificate contributes the full conditional-fibre product cost.
A cold certificate retains its exact window and is routed to the local
...
After the weighted package and hot-family aggregation of
\cref{lem:p13-window-package} have been constructed, the following counting
implication applies on
...
\begin{lemma}[Literal short self-return filter]\label[lemma]{lem:cold-short-self-return-filter}
Let \(J\) be an initial segment of a cold return corridor, starting at a
boundary stub attached to offset \(a\in\{0,\ldots,12\}\), and let \(x\) be
its current endpoint.  For an offset \(r\in\{0,\ldots,12\}\), suppose that
\(xv_r\in E(G)\).  Then \(J\), the two attachment edges, and the segment of
the induced \(P_{13}\) between \(v_a\) and \(v_r\) form a simple cycle of
length
\[
 |E(J)|+2+|a-r|.
\]
Consequently, if this length is a power of two for some offset with the stated
attachment edge, the corridor closes at case \textup{(F1)}.  If no such
offset exists, case \textup{(F1)} is absent at this stage and the same stage
is tested for \textup{(F2)}, \textup{(F3)}, and \textup{(F4)}.
\end{lemma}

\begin{proof}
The corridor prefix is a simple path in the graph obtained by deleting the
cold-window interiors, so its support is disjoint from the anchor window.  Its
initial boundary stub supplies the edge to \(v_a\), and the hypothesis
supplies the edge from \(x\) to \(v_r\).  Appending these edges and the unique
segment of the induced path between \(v_a\) and \(v_r\) gives a simple cycle.
The three pieces have lengths \(|E(J)|\), \(2\), and \(|a-r|\), respectively.
The final statement is the stage-major priority rule in
\cref{def:cold-corridor-first-failure}.
\end{proof}
...
The \emph{first failure} of \(\epsilon\) is computed in stage-major order.
Initial segments are read in their order along the corridor.  At a fixed
initial segment, the thirteen window offsets are inspected in the order
\(0,1,\ldots,12\) for \textup{(F1)}, after which \textup{(F2)},
\textup{(F3)}, and \textup{(F4)} are tested at that same segment:
\begin{enumerate}[label=(F\arabic*), leftmargin=2.4em]
\item the current endpoint has an actual attachment edge to a displayed
cold-window offset and the literal completion of
\cref{lem:cold-short-self-return-filter} has power-of-two length;
...
\item every initial segment is negative for \textup{(F1)}--\textup{(F4)}, and
either the corridor reaches its successor boundary stub before
\(Q_{\rm cold}+1\) states have been read, or a cold corridor state repeats.
\end{enumerate}
...
\begin{remark}[Pending cold semantic producer]\label[remark]{rem:cold-first-failure-producer}
The literal F1 constructor and the stage-major finite runner have been
formalized.  On the locally surviving branch, the structural corridor runner
also proves the exhaustive F5 support split: after exclusion of an actual F2
distinction, it returns either the whole bounded terminal corridor or the
first repeated structural-state span; an F3 compression is already excluded
by minimality.  The runner is not yet an unconditional implementation of node
[153]: it still requires the graph-owned composition of these pieces into one
stage-major F2--F5 producer, including the exact Type B or route-8 key for F4,
and it must promote the terminal or repeated support to the full two-boundary
cold bounded germ with its finite target-response table and replacement data.
These certificates are local to the stored corridor prefixes; their
construction does not enumerate the ambient context space.
\end{remark}
...
segments in stage-major order as specified in
\cref{def:cold-corridor-first-failure}.  At each segment, absence of every
literal \textup{(F1)} attachment advances to \textup{(F2)}, then
\textup{(F3)}, then \textup{(F4)}, before the next segment is read.  If the
successor boundary stub
...
If the first failure is (F1), its recorded attachment edge and corridor prefix
give the literal cycle of \cref{lem:cold-short-self-return-filter}; its length
is a power of two, which is impossible in a counterexample.  If it is (F2),
...
equal-length cold bounded germs and the short self-return rows that reach
exhaustive case \textup{(F5)} after the literal attachment test of
...
In particular, an equal-length cold bounded germ and an F5 short self-return
...
\(\delta\ge0\).  In this statement an achievable length is a literal
completion represented in the germ's exact response table; membership in an
arithmetic interval or residue class alone does not supply an attachment edge
or a graph cycle.  The following cases exhaust the finite arithmetic:
\begin{enumerate}[label=(\alph*), leftmargin=2.2em]
...
9 & Short self-return & F1 requires an actual endpoint--offset attachment and dyadic literal completion & stage-major local scan & F1, then same-stage F2/F3/F4; only exhaustive F5 enters the finite table \\
...
14 & \(\delta\ge13\) orbit hits a represented offset residue & \(2^k\equiv L+r\pmod\delta\), \(0\le r\le12\), with a literal compatible completion & target algebra & G1 \\
...
ambient-cubic filtering, inject every discarded non-cubic cold window into
the inherited surplus ledger, and prove the exact
\(15\)-stub/\(13\)-excess inequality, including the exact square-root loss
bound inherited from node [21].  They do not supply the remaining semantic
producers.
\end{remark}

\begin{remark}[Near-cubic $P_{13}$ window density bounds]\label[remark]{prop:p13-density}
On the live terminal at node~[149], the window-only conclusion is
\[
\theta\le
\theta_{\rm win}+o(1),
\qquad
...
=0.01270017798\ldots .
\]
The current Lean node-[22] constructor makes the exhaustive strict/at-most
comparison.  Node~[23] attaches the strict overflow inequality to the literal
yes constructor, without an auxiliary closure assumption, while node~[24]
consumes the literal no constructor and retains its exact finite ceiling and
cross-multiplied cap.  The chain remains conditional only on the typed
node-[21] producer, so the normalized conclusion displayed here is not yet
unconditional Lean evidence.
...
\]
The window-only node-[149] inequality is the intended consequence of the
weighted live-package aggregation once its conditional producers are
discharged.  The sharper high-entropy inequality and the strict-quarter cold
closure still require the corresponding terminal cold routes of
\cref{lem:p13-window-package,thm:cold-branch-quantitative-closure}.
The normalized high-entropy comparison would then be
\begin{equation}
...
A \emph{determination certificate} for \((a,\mathcal B)\), relative to an
original atom \(C\) carrying the declared coordinates, is a tuple
\[
\mathfrak c=(C,X,T_C,T_X,q,q_C,\mathcal P)
\]
with the following data.
\begin{enumerate}[label=(\alph*), leftmargin=2.2em]
\item \(C\subseteq X\subseteq G\), where \(X\) is a connected
\(T_X\)-boundaried support carrying all coordinates in
\(\{a\}\cup\mathcal B\), and \(T_C,T_X\) are the respective labelled
boundaries determined by the two supports.
\item \(q:\rho_{T_X}^{\rm ex}(X)\to Q\) is a functional admissible rank
quotient at the \(X\)-interface.
\item For any two realizations of \(\rho_{T_X}^{\rm ex}(X)\) with the same
...
\item \(q_C\) is the raw quotient proposal induced on the declared
coordinates of \(\rho_{T_C}^{\rm ex}(C)\).  The certificate records the
support inclusion, both boundary-degree profiles, the transport of every
declared coordinate used by \(q\), and the commutative equality saying that
\(q_C\) has the same quotient code as \(q\) on those transported labels.
No target-completeness assertion at the \(C\)-interface is included here; that
is exactly the subsequent context-universality test.
\end{enumerate}
Here a realization means a \(T_X\)-boundaried support whose exact response profile
...
Admissibility here is indexed by the final carrier \(X\).  When the proof later
asks whether this determination is already valid on a smaller atom
\(C\subseteq X\), it uses the boundary profile and outside-context space of
\(C\), not those of \(X\).  Thus context-universality at the \(X\)-interface
does not by itself imply context-universality at the \(C\)-interface.  The
restriction from \(X\) to \(C\), its boundary-profile transport, and the
corresponding context test are part of the determination certificate's routed
data.
\end{definition}
...
certificate has an admissible quotient \(q\) on its final carrier \(X\) and a
finite support set \(\mathcal P\).  Restrict the determination to the original
atom \(C\).  Either this restricted determination fails for some
\(\partial C\)-boundaried outside context, or it is context-universal at the
\(C\)-interface.  In the
...
that separates the two wedge coordinates has a witnessing closure which leaves
the corresponding closed ball.  This observation alone does not show that the
complete support of the tester is contained in the complement of the two balls:
a tester may meet a ball and then leave it.  The required remaining
normalization statement is that every separating tester can be replaced,
without changing either exact response, by a boundaried tester whose complete
context support is disjoint from both closed balls.  Equivalently, one may
prove by the same replacement strategy that every non-normalizable
intersection already yields a target defect or a proper compression.  This
normalization is an outstanding obligation of the present proof.

Conditional on this normalization, since the rooted balls are isomorphic and vertex-disjoint, the two local wedge
...
In the Lean realization, the node~[31] rank universe is the proof-carrying
support-stratified CT15 candidate family.  Hence the strict-loss circuit at
node~[35] already contains its determination certificate, including its
original atom, connected final carrier, exact quotient collision,
representative, and support-minimality data.  Node~[35] projects this
certificate directly from the accumulated node~[33] residual; it requires no
separate collision-to-support input.

\begin{corollary}[Conditional finite curvature magnitude and product cost]\label[corollary]{cor:forced-curvature-cost}
Assume the node-[24] packing cap and retain the node-[47] full-rank ledger on
the same remainder.  Put
\[
 L_n=\lfloor\log_2 n\rfloor S_{\rm hot},
\]
where $S_{\rm hot}$ is the fixed exact normalization scale of the hot
certificate, and let $E_{\rm norm}$ be the normalization error retained at
node~[24].  Writing $s(G)$ for the total degree surplus, one has the exact
finite inequality
\[
250825743018\,|R|L_n
 \le 98608581006\,r_\Omega(R)L_n
     +30E_{\rm norm}+197217162012\,s(G)L_n.
\]
On the high-entropy density branch the left coefficient may be replaced by
$253825743018$.  Since node~[25] gives $n\ge13$, the exact divisor $L_n$ is
positive.  Division by $L_n$ gives
\[
250825743018\,|R|
 \le 98608581006\,r_\Omega(R)+e_{48}(G),
\qquad
e_{48}(G)=\frac{30E_{\rm norm}}{L_n}+197217162012\,s(G),
\]
and the analogous high-entropy inequality.  More precisely, for every family
of these residuals in which the authored window, remainder, and primitive
thresholds are fixed and $n\to\infty$, the retained normalization envelope
gives $30E_{\rm norm}/L_n=o(n)$, while the near-cubic surplus certificate
gives $s(G)=O(\sqrt n)=o(n)$.  The node-[25] remainder estimate moreover
gives $|R|\ge n/4$ eventually.  Hence
\[
 e_{48}(G)=o(|R|).
\]

Multiplying the divided inequality by
\[
c_\Omega=\log_2(543958/111286)\ge0
\]
gives the stated window-only cost with
$K_{\rm win}=c_\Omega\omega_{\rm win}$ and, on the high-entropy edge, the
sharper cost with $K=c_\Omega\omega$.
\end{corollary}

\begin{proof}
The first inequality is the integer form of the wedge lower bound after the
error-bearing node-[24] density cap is substituted.  The generic scaled-wedge
calculation retains thirty copies of the density normalization error and twice
the retained total surplus at the common scale $L_n$.  The positivity of
$L_n$, the graph-order normalization and surplus envelopes, and the eventual
lower density $|R|\ge n/4$ give the displayed $o(|R|)$ error.  The sharper
coefficient is obtained by the same calculation from the high-entropy cap.
Finally, multiplication by the fixed nonnegative constant $c_\Omega$ carries
the $o(|R|)$ rank error to an $o(|R|)$ cost error.
\end{proof}

The two finite accounting inequalities required at node~[48] remain missing
obligations.  Positive scalar transport from either inequality to its
real-valued cost conclusion is proved, but that conditional transport is not
evidence for the missing finite producer.  Node~[48] is therefore partial.
...
unavailable in the main spine.  The value $c_\Omega$, by contrast, is the
logarithm of the exact finite count ratio retained in the typed node-[21]
output.  Node [48]
merely converts the already forced rank into these fixed budget units.
Construction and entropy of the remainder state family begin at node [49];
they are not additional obligations of node [48].
\end{remark}
...
\]
The atom part and the current net-deficiency cap are the exact branch-local
objects inherited with the fixed remainder.  Since every candidate has the
same vertex set, the density comparison is equivalently the comparison of its
net-deficiency numerator with that inherited numerator.  The family is used
symbolically; this definition does not prescribe an enumeration of labelled
graphs.
\end{definition}
...
\[
n^{|R|}\le |\mathcal G(R)|^{10},
\]
the node-[49] constrained remainder family satisfies
\[
\frac{|R|}{10}\log_2n\le\log_2|\mathcal G(R)|.
\]
\end{lemma}

\begin{proof}
The fixed remainder itself belongs to $\mathcal G(R)$, so
$|\mathcal G(R)|>0$; also the minimal counterexample has $n>0$ vertices.
Apply the increasing function $\log_2$ to the node-[50] inequality and use
the power law to obtain
\[
|R|\log_2n\le 10\log_2|\mathcal G(R)|.
\]
...
\begin{proof}
The exact curvature accounting at node [48] and the node-[49] constrained
remainder family give
\[
A\le B|\mathcal G(R)|.
\]
Raising to the tenth power and using the strict node-[50] low inequality
$|\mathcal G(R)|^{10}<U$ gives
\[
A^{10}\le (B|\mathcal G(R)|)^{10}
=B^{10}|\mathcal G(R)|^{10}<B^{10}U,
\]
...
The complementary no constructor of node [53] is exactly node [55]; the
strict-quarter cap is proved only at the subsequent node [56].
\end{proof}
...
$|\mathcal G(R)|^{10}<n^{|R|}$.  The radius-$r$ type coordinate is one canonical
...
n^{|R|/10}.
\]
For the formal proof this first sentence is a separate node-[52] obligation,
not a consequence of the definition of \(\eta(R)\).  Lean now proves the
required same-completion statement by a recoverable symbolic join.  Starting
from any realized remainder state, the hot aggregate reglues an arbitrary
dependent family of retained window choices while preserving the restricted
remainder graph.  Recovery of the remainder state and every window choice
makes the resulting map into the labelled skeleton code injective.  Hence
\[
 |\mathcal G_{\rm real}(R)|\,
 \left|\prod_{W\in\mathcal P_{\rm hot}}\mathcal S_W\right|
 \le |\mathcal C_{\rm skel}|.
\]
This is the exact finite node-[52] joint-capacity certificate.  It is symbolic:
no remainder family, Cartesian product, or completion universe is enumerated.
Failure to supply the required reglue operation is precisely the already
declared cold outcome, not a new branch.  Lean also instantiates the reusable
powered joint-normalization profile and proves the exact natural-number
inequality in which every discarded hot scale and every cold-window bit is
retained on the error side.  The framework then takes base-two logarithms and
node [52] substitutes exactly the node-[51] remainder-bit lower bound.  The
result is the error-bearing joint budget consumed by node [54]; the
error-free finite surrogate is not used.
...
The formal node-[54] consumer is the strict-cap consequence of the normalized
error-bearing inequality: it excludes the strict reverse comparison by
$A\le B\Rightarrow\neg(B<A)$.  The exact joint-capacity, powered
normalization, logarithmic transport, and node-[51] substitution are now
kernel-checked.  Thus node [52] supplies precisely the non-strict inequality
whose strict reverse is terminalized on the displayed $[52]\to[54]$ edge;
the usual near-cubic estimate merely rewrites its skeleton term in the
printed asymptotic notation.
...
For the Lean diagram interface, node~[53] is retained as the literal budget
dichotomy rather than identified with either outcome.  On the exact low
constructor of node~[50], it compares the remaining finite labelled-skeleton
bit budget (after the hot-window and node-[49] remainder demands) with the
full-rank curvature cost inherited from nodes~[47]--[48].  The framework keeps
the node-[51] high output unchanged and records both the strict small-budget
edge to node~[54] and the complementary large-budget edge to node~[55].
Node~[55] is the zero-copy name of that exact complementary leaf and adds no
new inequality.  The strict-quarter conclusion belongs solely to node~[56].
Its current local implementation proves the exact rational bound
\(\tau_{\rm win}<1/4\), while the finite error-bearing net-cap producer is
missing.  Node~[54], by contrast, is complete on both incoming edges.  On the
small node-[53] edge, the full-rank residual supplies the certified table
product
\[
  543958^r\le 111286^r N_R,
\]
with both factors tied to the audited node-[21] \((1,1)\)-entry and with
\(N_R\) the exact node-[49] realized-state count.  Raising this inequality to
the tenth power and using the strict node-[50] bound (N_R^{10}<n^{|R|})
contradicts the node-[53] small comparison.  The framework therefore
terminalizes the node-[52] high edge and the node-[53] small edge and retains
only the displayed node-[53] large residual toward node~[55].
\end{proof}
...
\node[box,text width=2.70cm] (pack) at (-1.95,-0.20) {\textbf{[52]} joint window--remainder\\accounting};
\node[good,text width=2.70cm] (cap) at (-1.95,-1.55) {\textbf{[54]} entropy cap\\closes};
...
The intended Lean realization first retains the displayed estimate in the
finite form
\[
\defp(R)-\sigma_R
\le \tau_{\rm win}|R|+E_{56}(R),
\]
where \(E_{56}\) is the sum of the exact node-[24] normalization correction
and the node-[29] total-surplus term.  At present the exact finite inequality
is a missing node-[56] obligation; the local node-[56] producer independently
kernel-checks the exact constant
\(\tau_{\rm win}<1/4\).  Migrating the earlier asymptotic ledger will prove
\(E_{56}(R)/|R|\to0\) on the eventual tail.  Thus this remains the intended
error-bearing form of node~[56], not unconditional or error-free evidence for
every finite order.
...
incidence bound used below.  These are the open node-[84] producer contract.
...
The intended finite node-[21] companion stores the same fifteen
\(399\times399\) Boolean
matrices as fixed bit rows.  Compact whole-row certificates identify every
stored row with the power-of-two attachment predicate, and bounded fixed-table
audit shards recheck every accepted safe and flat count against the generic
finite-relation formula.
...
This is the finite node~[21] certificate prescribed by the manuscript.  Its
current Lean producer remains partial until the exact carrier obligation is
removed.  The construction and routing of
...
\midrule
\endhead
{[23]} \texttt{overflow} & node~[22] yes constructor & the exact strict cross-multiplied window-load inequality & exact arithmetic & \textbf{Typed output, not an assumption.} The constructor is retained for the later hot/cold analysis. & $G$ carries the dense induced-$P_{13}$ packing inequality selected by node~[22].  Any later structural treatment of correlated or cold windows consumes this retained residual; it is not a premise used to manufacture node~[23]. \\
