...
\node[wide] (interface) at (0.00,7.50) {\textbf{[145]}~packing-order extension ledger:\\accept into aggregate / reject cold};
...
\node[wide] (germs) at (0.00,-5.35) {\textbf{[153]}~stage-major first failure:\\F1, then F2/F3/F4; exhaustive F5};
\node[dec] (germcase) at (0.00,-7.80) {\textbf{[154]}~exhaustive F5 bounded germ:\\G1, G2, or G3?};
\node[term] (hit) at (-6.25,-10.15) {\textbf{[155]}~F1/G1: literal\\dyadic cycle};
\node[term] (defect) at (0.00,-10.15) {\textbf{[156]}~F2/F4/G2: defect,\\exit (4), or handoff};
\node[term] (compress) at (6.25,-10.15) {\textbf{[157]}~F3/G3 or same-interface\\table: compression};
...
\caption[Proof-dependency diagram, Part XI]{Proof-dependency diagram, Part XI.  This panel preserves the original hot/cold \(P_{13}\)-window topology behind nodes [22]--[24].  If \(\theta<1/78\), route 8 closes at [147].  Otherwise the hot entropy cap yields [149] or a linear unpaid cold family [150], followed by the near-cubic and stub-excess stages [151]--[152].  Node [153] contains the graph-owned local attachment, return, component, and D1--D7 first-failure computations; only exhaustive F1--F4 absence enters the germ trichotomy [154] as F5.  The three original consumers remain [155], [156], and [157].}
...
\cref{lem:same-token-coarse-bottleneck-classification} & ENT\slash SURP & [144] \texttt{(coarse collision)} & green nodes [140], [142], [143] & Executes the exact 49-item/48-code scan and retains two distinct equal-code pairs, literal attachment maps, and two canonical rooted germs. & Routes the unchanged residual to the verified finite classifier [144] without asserting a semantic conclusion. \\
\cref{lem:same-token-semantic-bottleneck-classification,rem:same-token-semantic-bottleneck-classification-scope} & ENT\slash SURP & [144] \texttt{(finite classifier)} & green node [144] and its exact typed trigger & Scans the literal \(78p_{13}\) attachment coordinates, returns the first mismatch or full alignment, and then records one of four exact rooted-germ shapes in an exhaustive CT10 table. & Routes the unchanged five-way residual to [144]; it asserts no sparse-exit, CT3, Type B, fixed-cap, or spine conclusion. \\
...
The Lean interface verifies the variable-factor telescope, positive scale
...
Since node [21] retains the exact inequality
\(\sigma(G)^2\le K n\), the discarded loss also satisfies
\(|\mathcal P_{\rm cold}^{\rm noncub}|^2\le K n\).  This is the finite
square-root form of the paper's \(o(n)\) assertion and uses only the verified
node-[21] predecessor.
...
outcomes feed the separately verified internal node-[153] branch consumers, but
...
exact shorter residual is consumed by the verified proof-level rejoin [153]
and packed-support transition [153]; its two computed outcomes feed the
separately verified internal node-[153] branch consumers.
\end{remark}
...
consumed by the verified proof-level rejoin [153] and packed-support transition
[153]; its two computed outcomes feed the separately verified downstream
...
[153], whose exact all-inside branch subsequently feeds verified owner-change
node [153].  That internal support stage supplies no owner table or cross-window token,
...
verified owner-change node [153].  The transition output constructs one exact
stub, endpoint, and component, but no successor, second stub, or path through
that component; verified node [153] computes those objects on the exact
...
residuals feed the verified finite classifier internal to node [153].  This stage proves no CT3
...
The formal handoff processes the exact node-[21] packing in its declared
...
=0.01270017798\ldots .
\]
The Lean interface retains the exact node-[148] predecessor and the literal
finite natural-number normalization error.  The latter is bounded by a
graph-order-only envelope \(E(n)=o(n\log n)\), including both the powered
skeleton versus \(\operatorname{Nat.log}\) rounding and the discarded-scale
loss.  Dividing
the exact finite cap by its positive binary-log budget gives an explicit
additive term tending to zero, and hence the displayed node-[149] conclusion.
...
\]
The window-only node-[149] inequality now follows from the weighted
live-package aggregation.  The sharper high-entropy inequality and the
strict-quarter cold closure still require the corresponding terminal cold
routes of
...
If, in addition, a finite ordered family of pairwise distinct
graph-completion states, an injective code of those states into the labelled
baseline-skeleton family, and a conditional-fibre ledger for every curvature
coordinate are supplied on this same residual, then that ledger gives
\[
543958^{r_\Omega(R)}
 \le 111286^{r_\Omega(R)}\,N_{\rm skel},
\]
where $N_{\rm skel}$ is the exact number of labelled baseline skeletons.
Constructing this simultaneous graph-owned realization from node [47] is a
separate unproved obligation of node [48]; no complementary outcome or edge to
node [55] is introduced.
\end{corollary}
...
obtained by the same calculation from the high-entropy cap.  Under the stated
additional realization hypothesis, each local conditional-fibre inequality
\[
...
which gives the displayed product bound.  This conditional calculation does
not construct the realization.
\end{proof}
...
For any such supplied realization, the verified finite-capacity theorem gives
the corresponding product bound without materializing the Cartesian product.
The construction of the multiscale window schedule, joint commutation,
injective baseline encoding, and the authored demand bound remains the open
obligation at node [52].  No absence constructor or conditional edge to node
[55] is part of the original diagram.  The conditional capacity theorem does
not by itself assert the sharper density cap or the contradiction at node
[54]; those require the separate joint-capacity arithmetic displayed between
nodes [52] and [54].  Its computation enumerates no state product, graph
family, subset family, context universe, or Boolean cube.
\end{definition}
