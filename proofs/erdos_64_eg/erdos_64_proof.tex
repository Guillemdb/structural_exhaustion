...
logarithm of the exact finite count ratio certified at node [21].  Node [48]
...
not a consequence of the definition of \(\eta(R)\).  The latter counts the
predicate-defined candidate graphs in \(\mathcal G(R)\); it does not by itself
show that its cardinality is a realized target-complete contribution in the
same entropy comparison as the window coordinates.  Until this local
realization statement is established from the preceding residual ledger, the
following addition of the two inherited bit contributions remains the open
node-[52] handoff.  The Lean interface therefore gives node [52] only the
finite, error-bearing feasibility proposition on the accumulated node-[51]
high constructor.  The framework preserves the complementary low constructor
and every predecessor; node [52] introduces no completion universe, product
schedule, or additional branch.
...
The formal node-[54] consumer is exactly the strict-cap consequence of this
inequality: it excludes the strict reverse finite comparison by
$A\le B\Rightarrow\neg(B<A)$.  This implements the displayed edge
$[52]\to[54]$ without introducing a new case or enumerating any state family;
the graph-semantic producer of the node-[52] feasibility inequality remains a
separate obligation.
...
Node~[55] is implemented as that exact complementary leaf: it adds no new
inequality, and the corrected node-[24] density handoff is retrieved from the
same accumulated residual rather than copied into a new record.  The
strict-quarter conclusion remains solely the responsibility of node~[56].
That node is implemented by one framework ledger query which retrieves the
node-[25] positive normalization scale, the node-[29] surplus-adjusted
incidence supply, and the exact node-[55] leaf at the same residual.  The
framework's generic density--supply transport gives the finite cap with the
normalization and total-surplus errors explicit; the inherited near-cubic
envelope proves that error is (o(|R|)), and the exact rational computation
gives (	au_{m win}<1/4).  No predecessor fact is restated in the
node-[56] output.
\end{proof}
...
In the Lean realization, the displayed estimate is retained first in the
finite form
\[
\defp(R)-\sigma_R
...
and the node-[29] total-surplus term.  The fixed-threshold near-cubic ledger
proves (E_{56}(R)/|R|\to0), so the strict-quarter conclusion holds on the
eventual tail.  This is the error-bearing form of node~[56], not an
error-free assertion for every finite order.
