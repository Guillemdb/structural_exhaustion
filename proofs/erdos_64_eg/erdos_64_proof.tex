...
\node[wide] (fanmass) at (5.45,-6.95) {\textbf{[84]}~fan-mass route:\\certificate failures, unresolved local entries, and B2 failures charged to assigned surplus};
...
\cref{lem:exact-cubic-baseline-budget}, \cref{lem:incremental-skeleton-room} & ENT\slash SURP & auxiliary entropy audit & inv 10, 14, 15; \cref{def:baseline-spine-demand} & Computes the cubic baseline budget and the incremental edge-count room above cubic. & Supplies numerical room only when an independent realization contract is separately present. \\
\cref{prop:sparse-entropy-sandwich} & ENT\slash SURP & auxiliary entropy audit & inv 10, 14, 15; complete mixed Boolean-state realization; \cref{lem:exact-cubic-baseline-budget}, \cref{lem:incremental-skeleton-room} & Bounds blocker-free surplus-pair responses under the stated complete-realization contract. & Conditional auxiliary result; it is not an input to the node-[131] route. \\
\cref{prop:sparse-entropy-sandwich-with-blockers} & ENT\slash SURP & auxiliary entropy audit & inv 10, 14, 15; complete mixed Boolean-state realization; \cref{prop:sparse-entropy-sandwich} & Gives the corresponding conditional free-pair estimate in the presence of blockers. & Conditional auxiliary result; it is not an input to the blocker or total-token ledgers. \\
...
\cref{cor:spine-lower-bound-surplus-estimates} & ENT\slash SURP & auxiliary entropy audit & inv 14, 15; \cref{def:spine-lower-bound-deficits} & Converts a separately supplied complete Boolean realization into surplus estimates. & This conditional estimate is not an input to node [137] or [138]. \\
...
\begin{definition}[Complete mixed Boolean-state realization]\label[definition]{def:sparse-mixed-cube-realization}
Let \(\mathcal I\) be a finite family of declared Boolean target coordinates.
A \emph{complete Boolean-state realization} of \(\mathcal I\) consists of a
finite canonical family \(\mathcal S\) of target-complete states and a response
map
\[
\operatorname{val}:\mathcal S\longrightarrow\{0,1\}^{\mathcal I}
\]
such that, for every assignment
\(\varepsilon:\mathcal I\to\{0,1\}\), there is a state
\(s\in\mathcal S\) with \(\operatorname{val}(s)=\varepsilon\).  The states
are required to arise from the labelled graph class used in the skeleton
count.  For a baseline family \(\mathcal I_{\rm spine}\) and a free-pair set
\(\Pi_{\rm free}\), the \emph{mixed realization contract} is this datum for
\[
\mathcal I_{\rm spine}\cup\{r_\pi:\pi\in\Pi_{\rm free}\}.
\]
This is proof-carrying finite data.  Its verifier checks a supplied realization
map; it does not generate the Boolean cube or enumerate labelled graphs.
\end{definition}
...
\]
is label-injective under every admitted rank quotient.  A complete Boolean-state
realization in the sense of \cref{def:sparse-mixed-cube-realization} is a
separate condition.
\end{proposition}
...
for every admitted quotient.  Full realization of
\(\{0,1\}^{\binom{\mathcal A_0}{2}}\) is not a consequence of label
injectivity and is therefore retained as the separate contract of
\cref{def:sparse-mixed-cube-realization}.
\end{proof}
...
the pair-response family has a complete Boolean-state realization.  Then
\[
\binom{|\mathcal A_0|}{2}
\le
...
\begin{proof}
The complete Boolean-state realization makes all pair-response coordinates
indexed by \(\binom{\mathcal A_0}{2}\) independently target-testable.
\Cref{lem:independent-target-entropy,lem:skeleton-dominates}
therefore give
\[
...
\(E_{\rm spine}(n)\), and suppose that the mixed family
\[
\mathcal I_{\rm spine}\cup
\{r_{\{p,q\}}:\{p,q\}\in\tbinom{\mathcal A_0}{2}\}
\]
has a complete Boolean-state realization in the sense of
\cref{def:sparse-mixed-cube-realization}.  Then
\[
\binom{|\mathcal A_0|}{2}
\le
...
\begin{proof}
The supplied mixed realization makes the union
\[
\mathcal I_{\rm spine}\cup
\{r_{\{p,q\}}:\{p,q\}\in\binom{\mathcal A_0}{2}\}
\]
independently target-testable.  Therefore
...
deficit \(E_{\rm spine}(n)\).  Suppose also that
\[
\mathcal I_{\rm spine}\cup\{r_\pi:\pi\in\Pi_{\rm free}\}
\]
has a complete Boolean-state realization in the sense of
...
\begin{proof}
The mixed-realization contract applies to the declared coordinate family
\[
\mathcal I_{\rm spine}\cup\{r_\pi:\pi\in\Pi_{\rm free}\}.
\]
It therefore realizes every Boolean response vector and is independently
target-testable.  The definition of \(\Pi_{\rm free}\) separately excludes
every admitted structural blocker; no property of the blocked branch is used
