...
\node[wide] (realize91) at (6.25,7.50) {\textbf{[160]}~91-bit completion and\\commuting realization (open)};
...
16c & [160] & 91-bit realization obligation & construct the finite 91-coordinate completion states and prove commuting simultaneous realization & open; distinct from the actual 13-bit classifier & \cref{rem:p13-91-realization-obligation} \\
...
16r & [175], [180], [182] & D4--D7 availability, readiness, and semantic consumer & consume node [174]'s exact split, eliminate the impossible complete anchor reconstruction, and retain the exact missing clauses & [175] and [180] verified locally; full compatible-response and CT8 semantics remain open at [182] & \cref{lem:p13-component-d4d7-classifier,lem:p13-component-d4d7-semantic-readiness,rem:p13-component-d4d7-full-semantics} \\
...
\cref{lem:p13-actual-attachment-cold-fork,rem:p13-actual-attachment-fork-scope} & ENT\slash EXC & [21] $\to$ [158] \texttt{(actual cold)} $\to$ [159] & one exact selected window and target avoidance & Scans the literal outside-vertex-by-13 adjacency system, eliminates the hot outcome by a four-cycle, and retains the canonical missing assignment on the same window. & Feeds the graph-owned same-window frontier; the separate [160] 91-coordinate realization obligation remains open. \\
...
\cref{rem:p13-91-realization-obligation} & ENT & [21] $\to$ [160] $\to$ [22] \texttt{(open)} & finite 91-coordinate completion system and commuting gluing semantics & No producer is currently proved. & White obligation; no fact from the 13-bit branch is substituted. \\
...
remains the separate white 91-bit realization obligation.
\end{remark}
...
Node [194] is the verified terminal residual interface for node [191].  A
future closure proof must construct its three graph-owned inputs from the
retained local clauses and may not enumerate ambient responses, contexts,
states, graphs, or universes.
\end{remark}
...
provenance.  Each mismatch leaf records the two graph-owned inputs needed for
a distinguishing-response route.  The aligned leaf records compatible-response
semantics, its provenance, and a CT8 removal or distinguishing-route input.
These are obligation tags only.  The construction inspects one local
constructor and proves no response equivalence, distinguishing context, or
removal.
\end{lemma}

\begin{remark}[Open full long-prefix semantics]\label[remark]{rem:p13-long-prefix-full-semantics}
Node [195] is the verified terminal residual interface for node [192].  A
future closure proof must construct the missing compatible D4--D7 response
layer and then provide either a certified CT8 removal or a typed
distinguishing-context route.  It may inspect only explicit local clauses and
may not enumerate an ambient response, context, state, graph, or universe
family.
\end{remark}

\begin{remark}[Open 91-bit realization obligation]\label[remark]{rem:p13-91-realization-obligation}
Node [160] denotes the still-white edge from [21] to [22]: one must construct
the finite 91-coordinate completion-state system and prove that independently
chosen local responses commute in a simultaneous admissible realization.
This obligation is logically distinct from nodes [158]--[159].  A missing
thirteen-bit adjacency assignment neither proves nor refutes it.
\end{remark}
...
\le1.5+o(1).
\end{equation}
The displayed numerical bounds are therefore bookkeeping consequences, not
current inputs to the remainder branch.  Node~[24] remains open until it
constructs an exact finite ceiling \(U_{13}\) and proves \(p_{13}\le U_{13}\)
from the completed hot/cold route.
\end{remark}
