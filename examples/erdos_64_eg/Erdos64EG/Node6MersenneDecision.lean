...
def node6Input {V : Type u} {residual : InitialResidual V}
    (node5 : Node5Stage residual) : CT1.Input (problem V) :=
  ct1Input node5.previous.context.G node5.previous.context.baseline

/-- Node `[6]` is the framework-owned CT1 decision on the literal node-[5]
payload. -/
abbrev Node6Stage {V : Type u} (residual : InitialResidual V) :=
  Core.ResidualRefinement.State.DependentSuccessor (@Node5Stage V)
    (fun current node5 =>
      CT1.ResidualRefinement.CertificateDecision (mersenneReturnEncoding V)
        (node6Input (residual := current) node5))
    residual
...
noncomputable def node6MersenneDecision {V : Type u} {facts}
  ...
  CT1.ResidualRefinement.executeCertificateUsingStage
...
theorem node6_total {V : Type u} {residual : InitialResidual V}
    ...
    Nonempty (CT1.ResidualRefinement.CertificateDecision
      (mersenneReturnEncoding V) (node6Input node5)) :=
