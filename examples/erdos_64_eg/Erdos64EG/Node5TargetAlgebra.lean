...
abbrev Node5Stage {V : Type u} (residual : InitialResidual V) :=
  Core.ResidualRefinement.State.DependentSuccessor
    (@Node4Output V)
    (fun _residual node4 => Node5TargetAlgebra node4.context)
    residual
...
noncomputable def node5TargetAlgebra {V : Type u} {facts}
  ...
  Core.ResidualRefinement.State.StageNode.mapStage fun _residual node4 =>
    { targetIffReturn := target_iff_hasMersenneReturn node4.context.G
      returnSetsDisjoint :=
        (not_target_iff_returnSets_disjoint node4.context.G).mp
          node4.context.avoids }
...
abbrev Node5TargetAlgebraFact {V : Type u}
  ...
  ∃ node4 : Node4Output residual,
    (Target node4.context.G ↔
        HasMersenneReturn node4.context.G.graph) ∧
      ∀ dart : node4.context.G.graph.Dart,
        Disjoint (returnSet node4.context.G.graph dart) MersenneSet
