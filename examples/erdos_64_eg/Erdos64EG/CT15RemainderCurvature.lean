...
/-- The manuscript-exact node-[31] rank universe.  A candidate chooses its
connected boundaried carrier, exact-profile realization semantics, quotient
image map, and raw code, then proves declared support, boundary preservation,
context universality, and the proper/whole representative clause. -/
...
/-- Canonical manuscript rank before the atomic nodes-[31]--[35] consumer
migration.  This name is kept distinct from the old surrogate so no downstream
node can silently mix the two candidate universes. -/
