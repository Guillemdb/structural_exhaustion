...
/-!
# Represented 2D Navier--Stokes row-9 profile-family packet

The represented row-8 packet is inactive because the row-7 positive-capacity
focus is inactive.  Row 9 is still instantiated over the literal row-8
successor.  Core skips the CT17-to-CT12-to-CT11 payload on the inactive
successor and retains the predecessor ledger.
-/

namespace HypostructurePDEExamples.RepresentedNS2DProfileFamilyPacket

open Hypostructure
open Hypostructure.Core
open Hypostructure.Core.Residual
open Hypostructure.PDE.FastTrack
open HypostructurePDEExamples.RepresentedNS2DExactResponseCoveragePacket

noncomputable section

abbrev RowEightProfile := rowEightProfile
abbrev RowEightStage := ProfileFamily.RowEightStage RowEightProfile
abbrev RowNineView := ProfileFamily.ActiveView RowEightProfile

def rowEightStage : RowEightStage :=
  rowEightRun.value

theorem rowEightSuccessorInactive :
    Not ((ExactResponseCoverage.SuccessorFocus RowEightProfile).Active
      rowEightStage) := by
  intro active
  change
    (CapacityProfile.PositiveCapacityFocus RowSevenProfile).Active
      rowEightRun.value.previous at active
  rw [rowEight_retains_literal_row_seven] at active
  exact rowEight_positive_capacity_inactive active
...
abbrev rowNineProfile : ProfileFamily.Profile RowEightProfile :=
...
theorem rowNine_skips_inactive_payload :
    rowNineRun.checks =
      (ExactResponseCoverage.SuccessorFocus
        RowEightProfile).selectionBudget.checks rowEightStage :=
  rowNineContract.run_checks_of_inactive rowEightStage
    rowEightSuccessorInactive
...
theorem rowNine_retains_root_residual :
    Core.Residual.residualOf rowNineRun.value =
      HypostructurePDEExamples.RepresentedNS2DLocalTailPacket.zeroRootResidual :=
  by
    change Core.Residual.residualOf rowNineRun.value.previous =
      HypostructurePDEExamples.RepresentedNS2DLocalTailPacket.zeroRootResidual
    rw [rowNine_retains_literal_row_eight]
    exact rowEight_retains_root_residual

#print axioms rowNine_skips_inactive_payload
#print axioms rowNine_work_is_bounded

end

end HypostructurePDEExamples.RepresentedNS2DProfileFamilyPacket
