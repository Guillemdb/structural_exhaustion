...
structure BoundaryPiece (boundary : Boundary) where
  Internal : Type u
  internalVertices : FinEnum Internal
  graph : SimpleGraph (boundary.Vertex ⊕ Internal)
  decideAdj : DecidableRel graph.Adj

structure OutsideContext (boundary : Boundary) where
  Internal : Type u
  internalVertices : FinEnum Internal
  graph : SimpleGraph (boundary.Vertex ⊕ Internal)
  decideAdj : DecidableRel graph.Adj
...
Either side may own boundary-to-boundary edges. Gluing identifies equally
labelled boundary vertices and takes the union of the two edge sets, exactly as
in `original_erdos_64_proof.tex`; ownership may overlap and reconstruction is
certified against that union.

`Graph.BoundaryOverlap` computes the boundary graph owned by both sides and
proves exact inclusion--exclusion for glued boundary degrees and edge counts.
It derives final-degree equality only from local profile equality together
with equality of the actual overlap-degree profile against the context. It
derives global lexicographic progress from local progress only when overlap
edge counts agree. These premises are deliberately visible: unrestricted
union gluing does not permit either conclusion from local degree totals alone.
...
- the outside boundaried graph as the context;
...
`Graph.Boundary`, `Graph.Gluing`, and `Graph.BoundaryOverlap` contain the graph
mathematics for this adapter. Graph owns embeddings, edge ownership,
reconstruction isomorphisms, degree inclusion--exclusion, and exact size
change. Core owns:
...
### 6.1 Registered atom response coordinates

`Graph.AtomResponse.CoordinateSystem` is indexed by the exact
`BoundariedAtomProfileCertificate` already stored in the predecessor ledger.
It contains an arbitrary coordinate carrier, the boundary-degree profile of
each coordinate, its realization against every literal outside context, and a
proof that it lies in the registered profile fibre. Coordinates need not be
graphs: trace lengths, rooted-return tests, curvature tests, and other marked
response data all fit the same contract.

`TargetCompleteQuotient` accepts any setoid certified to preserve the target
response against all outside contexts. Graph derives profile preservation from
the registered fibre, projects every identification to context universality,
and provides the canonical maximal quotient generated by exact all-context
response equivalence. `CoordinateSystem.ofPieces` is only a specialization for
coordinate collections that are literally represented by boundary pieces.

The application does not copy the atom profile, implement quotient routing, or
prove context universality at Node 12. An arbitrary quotient is a quantified
input of the original lemma; its target-completeness certificate is the exact
hypothesis from which Graph projects the conclusion.

### 6.2 Proper atom and boundary-profile registration
...
structure BoundariedAtomRegistration (context : MinimalContext) where
  family : forall atom : ProperBoundariedAtom context.G,
...
def executeFocusedBoundariedAtomRegistration ... :=
  (executeFocusedBoundariedAtomRegistrationCounted ...).value
```

Graph derives lexicographic decrease and the proper-subgraph certificate from
the intrinsic statement that the embedded side does not contain both all
ambient vertices and all ambient edges. It also derives the exact uncapped
degree vector and coordinate equality, instantiates the generic
response-completeness contract with that vector, rejects identifications across
different profile fibres, and records the proof-only work count.
`Core.Residual.Focus.runCounted` decides activity once, stores its exact count,
preserves the literal predecessor, and performs the sole ledger extension. The
application supplies neither a
strict-progress proof, profile function, profile-fibre theorem, work claim, nor
successor payload.

`focusedBoundariedAtomMetadata` is the corresponding proof-relevant audit
record. It stores the exact active context query, generated registration and
stage outputs, generic derivation theorems, the inherited focus-selection
budget, and an empty manual-obligation list;
`focusedBoundariedAtomMetadataComplete` certifies that list is empty. Local
profile derivation performs zero checks; the total executor count is exactly
the focus selector's registered count for both active and inactive outcomes.
The same counted-first surface is mandatory for Graph CT1, proper-subgraph
minimality, deletion criticality, and slack-vertex independence. Their public
stage functions are projections of the counted result, not independent
executions.
...
The outside-context type permits either side to own boundary--boundary edges;
gluing follows the original definition by identifying boundary labels and
taking the union of both edge sets.
