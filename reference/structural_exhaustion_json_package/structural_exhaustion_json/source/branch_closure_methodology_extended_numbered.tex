...
\item
  \textbf{external types} play the role of Myhill--Nerode classes and of
  types in logic: two pieces have the same external type when no
...
\bottomrule\noalign{}
\endlastfoot
CT1 & Convert target realization into C1 or target-avoiding payloads. & Local test family, witness spaces, and target equivalence recorded. & Test family, witness extractor, branch-state record. & Reject missing S-Equiv; write target-avoidance row and payload row. & No local test family exists. \\
...
\bottomrule\noalign{}
\endlastfoot
CT1 & \(\langle\mathfrak B,T,\mathcal T,W\rangle\) & S-Def\(\langle\mathcal T,W\rangle\); S-Equiv\(\langle T,\mathcal T\rangle\); S-Pers\(\langle T\)-avoidance\(\rangle\); S-Rout; S-Trig. & C1, realized local test. & Target-avoiding payloads to CT2, CT3, CT4, CT5, CT6, CT17. & --- \\
CT2 & \(P_{1\to2}\) & S-Def\(\langle X,\iota_X,\prec\rangle\); S-Dich (delete, replace, survive); S-Comp\(\langle\prec\rangle\); S-Rout; S-Trig. & C2, minimality contradiction. & --- & Context residual to CT3; target-uncompressible datum to CT10. \\
...
CT2 & S-Def, S-Dich, S-Comp, S-Rout, S-Trig. \\
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the target predicate \(T\), the local test family \(\mathcal T\), the witness spaces \(W_i(G)\), and the witness extractor recorded in the branch state.
\item \textbf{S-Equiv} is instantiated with the equivalence between \(T\) and the existence of a realized local test \(T_i\).
...
\item \textbf{S-Rout} is instantiated with the target-avoiding payloads \(P_{1\to2}\), \(P_{1\to3}\), \(P_{1\to4}\), \(P_{1\to5}\), \(P_{1\to6}\), and \(P_{1\to17}\).
\item \textbf{S-Trig} is instantiated with the consumer trigger for each target-avoiding payload emitted by CT1.
\end{itemize}
...
is claiming target avoidance without S-Equiv; the audit records it as a
CT1/S-Equiv defect.
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N01}}}] (target) at (0,0.60) {\textbf{Definition}\\target predicate \(T\)};
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N02}}}] (tests) at (0,-1.55) {\textbf{Definition}\\local test family \(\mathcal T=\{T_i\}_{i\in I}\)};
\node[bcwide,bcdefinition,text width=4.30cm,minimum width=4.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N03}}}] (witness) at (0,-3.75) {\textbf{Definition}\\candidate witness spaces \(W_i(G)\)};
\node[bcauditbox,text width=4.20cm,minimum width=4.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N04}}}] (scope) at (0,-5.95) {\textbf{Lemma}\\bounded radius/interface; finite type index};
\node[bcscopefail,text width=3.15cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N05}}}] (scopebad) at (7.35,-5.95) {\textbf{Scope candidate}\\no local test family\\\scoperef};
\node[bcdec,bclemma,text width=3.25cm,minimum width=3.95cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N06}}}] (complete) at (0,-8.55) {\textbf{Lemma}\\\(T\) exists iff some \(T_i\) is realized};
\node[bcbox,bcdesign,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N07}}}] (testgap) at (-7.35,-8.55) {\textbf{Repair datum}\\bounded witness missing from \(\mathcal T\)};
\node[bcbox,bclemma,bcdesign,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N08}}}] (testlemma) at (-7.35,-11.25) {\textbf{Lemma}\\extend \(\mathcal T\) by the missing witness type};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N09}}}] (testroute) at (-7.35,-14.15) {\textbf{CT 1}\\local target tests};
\node[bcdec,bclemma,text width=3.35cm,minimum width=4.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N10}}}] (hitq) at (0,-12.05) {\textbf{Dichotomy lemma}\\target realization vs.\ target-avoiding state};
\node[bcclosed,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N11}}}] (hit) at (8.35,-12.05) {\textbf{C1}\\realized test gives target closure};
\node[bcbox,bcdefinition,text width=3.75cm,minimum width=3.75cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N12}}}] (avoid) at (0,-14.85) {\textbf{Branch-state definition}\\\(\mathfrak B_{\neg T}\): all tests absent};
\node[bcbox,bclemma,text width=3.75cm,minimum width=3.75cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N13}}}] (register) at (0,-17.25) {\textbf{Lemma}\\persistence of \(T_i\)-absence};
\node[bcbox,bclemma,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N14}}}] (next) at (0,-20.15) {\textbf{Payload-classification lemma}\\certified surviving data map to declared payloads};
\node[bcbox,bclemma,bcpayload,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N15}}}] (minlemma) at (-10.00,-23.50) {\textbf{Payload lemma}\\build \(P_{1\to2}\)\\proper piece};
\node[bcbox,bclemma,bcpayload,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N16}}}] (typelemma) at (-6.00,-23.50) {\textbf{Payload lemma}\\build \(P_{1\to3}\)\\boundary datum};
\node[bcbox,bclemma,bcpayload,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N17}}}] (chargelemma) at (-2.00,-23.50) {\textbf{Payload lemma}\\build \(P_{1\to4}\)\\charge map};
\node[bcbox,bclemma,bcpayload,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N18}}}] (sitelemma) at (2.00,-23.50) {\textbf{Payload lemma}\\build \(P_{1\to5}\)\\local sites};
\node[bcbox,bclemma,bcpayload,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N19}}}] (activelemma) at (6.00,-23.50) {\textbf{Payload lemma}\\build \(P_{1\to6}\)\\activity setup};
\node[bcbox,bclemma,bcpayload,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N20}}}] (thicklemma) at (10.00,-23.50) {\textbf{Payload lemma}\\build \(P_{1\to17}\)\\sparse offsets};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=2.90cm,minimum width=2.90cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N21}}}] (minroute) at (-10.00,-26.75) {\textbf{CT 2}\\minimality / replacement};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=2.90cm,minimum width=2.90cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N22}}}] (typeroute) at (-6.00,-26.75) {\textbf{CT 3}\\external types};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=2.90cm,minimum width=2.90cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N23}}}] (chargeroute) at (-2.00,-26.75) {\textbf{CT 4}\\charging};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=2.90cm,minimum width=2.90cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N24}}}] (siteroute) at (2.00,-26.75) {\textbf{CT 5}\\local-to-global};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=2.90cm,minimum width=2.90cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N25}}}] (activeroute) at (6.00,-26.75) {\textbf{CT 6}\\active/dormant};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=2.90cm,minimum width=2.90cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT1.N26}}}] (thickroute) at (10.00,-26.75) {\textbf{CT 17}\\target thickening};
...
\draw[bchandoff] (minlemma)--node[bclabel,right]{S-Trig for CT2}(minroute);
\draw[bchandoff] (typelemma)--node[bclabel,right]{S-Trig for CT3}(typeroute);
\draw[bchandoff] (chargelemma)--node[bclabel,right]{S-Trig for CT4}(chargeroute);
\draw[bchandoff] (sitelemma)--node[bclabel,right]{S-Trig for CT5}(siteroute);
\draw[bchandoff] (activelemma)--node[bclabel,right]{S-Trig for CT6}(activeroute);
\draw[bchandoff] (thicklemma)--node[bclabel,right]{S-Trig for CT17}(thickroute);
\end{tikzpicture}%
}
...
\item \textbf{S-Dich} is instantiated with the three alternatives that \(X\) can be deleted, replaced, or kept as a surviving residual.
...
\item \textbf{S-Rout} is instantiated with \(P_{2\to3}\) for a context-dependent response and \(P_{2\to10}\) for a target-uncompressible survivor.
\item \textbf{S-Trig} is instantiated with the CT3 trigger for context residuals and the CT10 trigger for target-uncompressible data.
\end{itemize}
...
universality is proved; the defect register records the missing CT2/S-Def
or CT2/S-Equiv obligation.
...
\node[bcwide,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N01}}}] (input) at (0,0) {\textbf{Assumption}\\minimal counterexample \(G\) under a fixed well-order};
\node[bcbox,bcdefinition,text width=3.60cm,minimum width=3.60cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N02}}}] (piece) at (0,-2.30) {\textbf{Definition}\\admissible piece \(X\) with interface data};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N03}}}] (scopebad) at (7.45,-2.30) {\textbf{Scope candidate}\\unbounded interface\\\scoperef};
\node[bcdec,bclemma,text width=3.25cm,minimum width=4.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N04}}}] (delete) at (0,-5.35) {\textbf{Lemma}\\deletion preserves hypotheses and target avoidance?};
\node[bcclosed,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N05}}}] (delout) at (7.45,-5.35) {\textbf{C2}\\smaller counterexample contradiction};
\node[bcbox,text width=3.60cm,minimum width=3.60cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N06}}}] (critical) at (0,-8.45) {\textbf{Branch-state record}\\\(X\) is deletion-critical};
\node[bcbox,bcdefinition,text width=3.60cm,minimum width=3.60cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N07}}}] (profile) at (0,-10.80) {\textbf{Definition}\\target-response profile \(\tau(X)\)};
\node[bcaudit,text width=3.25cm,minimum width=4.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N08}}}] (context) at (0,-14.05) {\textbf{Lemma}\\valid in every compatible outside context?};
\node[bcdec,bclemma,text width=3.25cm,minimum width=4.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N09}}}] (replace) at (0,-17.35) {\textbf{Lemma}\\smaller \(X'\) with \(\tau(X')\subseteq\tau(X)\)?};
\node[bcclosed,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N10}}}] (repout) at (7.45,-17.35) {\textbf{C2}\\replacement contradicts minimality};
\node[bcbox,bcdesign,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N11}}}] (ctxres) at (-6.35,-17.35) {\textbf{Payload record}\\\(P_{2\to3}\): context-dependent response};
\node[bcbox,bclemma,bcdesign,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N12}}}] (ctxlemma) at (-6.35,-20.45) {\textbf{Lemma}\\\(P_{2\to3}\) satisfies CT3 trigger};
\node[bcbox,bccorollary,text width=3.60cm,minimum width=3.60cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N13}}}] (uncomp) at (0,-20.55) {\textbf{Corollary}\\survivor is deletion-critical\\and target-\\uncompressible};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.30cm,minimum width=3.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N14}}}] (ctxroute) at (-6.35,-23.55) {\textbf{CT 3}\\external types};
\node[bcroutechoice,bctheorem,bcdesign,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N15}}}] (route) at (0,-23.55) {\textbf{Routing theorem}\\what data is now available?};
\node[bcbox,bclemma,bcdesign,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N16}}}] (critlemma) at (-4.70,-27.05) {\textbf{Payload lemma}\\build \(P_{2\to10}\)\\criticality refinement};
\node[bcbox,bclemma,bcdesign,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N17}}}] (resplemma) at (4.70,-27.05) {\textbf{Payload lemma}\\build \(P_{2\to3}\)\\missing response datum};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N18}}}] (next) at (-4.70,-30.35) {\textbf{CT 10}\\refinement};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT2.N19}}}] (typeroute) at (4.70,-30.35) {\textbf{CT 3}\\external types};
...
\draw[bcrecoverarr] (ctxlemma)--node[bclabel,right]{S-Trig for CT3}(ctxroute);
...
\draw[bcrecoverarr] (critlemma)--node[bclabel,right]{S-Trig for CT10}(next);
\draw[bcrecoverarr] (resplemma)--node[bclabel,right]{S-Trig for CT3}(typeroute);
\end{tikzpicture}%
}
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the certified interface \(\iota\), the external-type slot \(\tau\), and the response evaluator used by the current branch state.
\item \textbf{S-Equiv} is instantiated with the statement that \(\tau\) decides every compatible target-relevant context for the piece under analysis.
...
\item \textbf{S-Comp} is instantiated with the relevant type inclusion, finite type-table exhaustion, or compression comparison.
\item \textbf{S-Rout} is instantiated with \(P_{3\to7}\), \(P_{3\to12}\), and \(P_{3\to8}\) for exchange defects, peelable load, and long type sequences.
\item \textbf{S-Trig} is instantiated with the CT7, CT12, and CT8 triggers for those three outgoing payloads.
\end{itemize}
...
components; the audit records CT3/S-Equiv or CT3/S-Rout.
...
\node[bcwide,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N01}}}] (piece) at (0,0) {\textbf{Input}\\boundaried piece \(X\) with interface data};
\node[bcwide,bcdefinition,text width=4.30cm,minimum width=4.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N02}}}] (type) at (0,-2.15) {\textbf{Definition}\\external type \(\tau(X)\)};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N03}}}] (scopebad) at (8.35,-2.15) {\textbf{Scope candidate}\\infinite type index\\\scoperef};
\node[bcwide,bcdefinition,text width=4.30cm,minimum width=4.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N04}}}] (replacements) at (0,-4.45) {\textbf{Definition}\\candidate replacement family \(\mathcal R(X)\)};
\node[bcdec,bclemma,text width=3.35cm,minimum width=4.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N05}}}] (compare) at (0,-7.25) {\textbf{Lemma}\\replacement-inclusion test};
\node[bcclosed,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N06}}}] (compress) at (8.35,-7.25) {\textbf{C2}\\globally valid replacement};
\node[bcdec,bclemma,text width=3.35cm,minimum width=4.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N07}}}] (distinguish) at (0,-10.55) {\textbf{Dichotomy lemma}\\separating context vs.\ indistinguishability};
\node[bcroutechoice,bctheorem,text width=2.75cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N08}}}] (defectlemma) at (7.10,-10.55) {\textbf{Routing theorem}\\which defect payload is certified?};
\node[bcclosed,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N09}}}] (routedclose) at (12.35,-10.55) {\textbf{C3}\\defect account already routed};
\node[bcbox,bclemma,bcpayload,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N10}}}] (exchlemma) at (6.15,-13.65) {\textbf{Payload lemma}\\build \(P_{3\to7}\)\\separating-context defect};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N11}}}] (defect) at (6.15,-16.70) {\textbf{CT 7}\\exchange};
\node[bcbox,bclemma,bcpayload,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N12}}}] (peellemma) at (11.15,-13.65) {\textbf{Payload lemma}\\build \(P_{3\to12}\)\\load plus measure};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N13}}}] (peelroute) at (11.15,-16.70) {\textbf{CT 12}\\peeling loop};
\node[bcbox,bcdefinition,text width=3.85cm,minimum width=3.85cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N14}}}] (residual) at (0,-13.65) {\textbf{Branch-state definition}\\unresolved external-type residual};
\node[bcbox,bclemma,text width=3.85cm,minimum width=3.85cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N15}}}] (persist) at (0,-16.15) {\textbf{Lemma}\\type-data persistence};
\node[bcroutechoice,bctheorem,text width=2.75cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N16}}}] (next) at (0,-19.10) {\textbf{Routing theorem}\\what finite type structure remains?};
\node[bcbox,bclemma,bcpayload,text width=3.60cm,minimum width=3.60cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N17}}}] (repeatlemma) at (-5.10,-22.55) {\textbf{Payload lemma}\\build \(P_{3\to8}\)\\repeated-type sequence};
\node[bcbox,bclemma,text width=3.60cm,minimum width=3.60cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N18}}}] (tablelemma) at (5.10,-22.55) {\textbf{Lemma}\\finite-table certificate};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N19}}}] (pumproute) at (-5.10,-25.85) {\textbf{CT 8}\\pumping};
\node[bcclosed,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT3.N20}}}] (tableclose) at (5.10,-25.85) {\textbf{C5}\\finite type table closes};
...
\draw[bchandoff] (exchlemma)--node[bclabel,right]{S-Trig for CT7}(defect);
\draw[bchandoff] (peellemma)--node[bclabel,right]{S-Trig for CT12}(peelroute);
...
distinguish them with respect to the target. This is the exact analogue
of a Myhill--Nerode class, or of a type in logic, and the
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the demand set \(\mathcal D\), payer set \(\mathcal P\), charge map \(\pi\), capacity data, and lower-bound target.
\item \textbf{S-Det} is instantiated with the canonical definition of \(\pi\), including every tie-break used to choose a payer.
\item \textbf{S-Dich} is instantiated with the alternatives that certified capacity closes the branch or an overload fibre remains.
\item \textbf{S-Comp} is instantiated with the bounded-capacity inequality comparing total demand with available payer capacity.
\item \textbf{S-Rout} is instantiated with \(P_{4\to9}\) for an overcharged fibre, \(P_{4\to14}\) for aggregate residual mass, and \(P_{4\to13}\) for a demand with no available payer.
\item \textbf{S-Trig} is instantiated with the CT9 trigger for overload data, the CT14 trigger for aggregate mass data, and the CT13 trigger for a tier-interaction obstruction.
\end{itemize}
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N01}}}] (input) at (0,0.70) {\textbf{Definition}\\demand set \(\mathcal D\) and scarce payer set \(\mathcal P\)};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N02}}}] (scopebad) at (7.85,0.70) {\textbf{Scope candidate}\\no admissible local demand family\\\scoperef};
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N03}}}] (assign) at (0,-1.85) {\textbf{Definition}\\canonical assignment rule \(\pi:\mathcal D\rightharpoonup\mathcal P\)};
\node[bcbox,bcdesign,bcpayload,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N04}}}] (nopayer) at (9.15,-3.70) {\textbf{Payload record}\\\(P_{4\to13}\): no available payer};
\node[bcbox,bclemma,bcdesign,bcpayload,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N05}}}] (tierrepair) at (9.15,-6.60) {\textbf{Payload lemma}\\\(P_{4\to13}\) satisfies CT13 trigger};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N06}}}] (tierroute) at (9.15,-9.50) {\textbf{CT 13}\\tiered charging};
\node[bcauditbox,text width=3.75cm,minimum width=3.75cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N07}}}] (canon) at (0,-4.45) {\textbf{Lemma}\\\(\pi\) is total, deterministic, and branch-state measurable?};
\node[bcdec,bclemma,text width=3.20cm,minimum width=4.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N08}}}] (mult) at (0,-8.10) {\textbf{Dichotomy lemma}\\are all fibres \(C\)-bounded?};
\node[bcbox,bclemma,bcpayload,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N09}}}] (overtrig) at (-5.65,-10.70) {\textbf{Payload lemma}\\build \(P_{4\to9}\)\\overcharged fibre};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N10}}}] (overload) at (-5.65,-13.80) {\textbf{CT 9}\\overload};
\node[bcbox,bclemma,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N11}}}] (cap) at (0,-10.70) {\textbf{Lemma}\\capacity inequality\\\(|\mathcal D|\le C|\mathcal P|+o(n)\)};
\node[bcbox,bclemma,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N12}}}] (lower) at (5.65,-10.70) {\textbf{Lemma}\\demand lower bound\\\(|\mathcal D|\ge cn-o(n)\)};
\node[bcdec,bctheorem,text width=3.05cm,minimum width=3.90cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N13}}}] (contr) at (0,-14.00) {\textbf{Comparison theorem}\\does demand exceed capacity?};
\node[bcclosed,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N14}}}] (out) at (8.05,-14.00) {\textbf{C4}\\charging contradiction};
\node[bcbox,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N15}}}] (open) at (0,-17.05) {\textbf{Branch-state record}\\capacity bound remains compatible};
\node[bcbox,bclemma,bcpayload,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N16}}}] (masslemma) at (0,-19.55) {\textbf{Payload lemma}\\build \(P_{4\to14}\)\\aggregate mass residual};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT4.N17}}}] (aggregate) at (0,-22.55) {\textbf{CT 14}\\aggregate closure};
...
\draw[bcrecoverarr] (tierrepair)--node[bclabel,right]{S-Trig for CT13}(tierroute);
...
\draw[bchandoff] (overtrig)--node[bclabel,right]{S-Trig for CT9}(overload);
...
\draw[bchandoff] (masslemma)--node[bclabel,right]{S-Trig for CT14}(aggregate);
\end{tikzpicture}%
}
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the local-site family \(\mathcal S\), its site labels, the witness extractor, and the locality record.
\item \textbf{S-Det} is instantiated with the canonical site-to-payer assignment used when local witnesses form a charge map.
\item \textbf{S-Comp} is instantiated with the local-to-global summation and the resulting global capacity comparison.
\item \textbf{S-Rout} is instantiated with \(P_{5\to4}\), \(P_{5\to11}\), and \(P_{5\to14}\) for charge ledgers, additive deficits, and local-mass shortfalls.
\item \textbf{S-Trig} is instantiated with the CT4, CT11, and CT14 triggers for the outgoing local-to-global payloads.
\end{itemize}
...
execution error is a missing locality lemma; the audit records CT5/S-Def
or CT5/S-Comp.
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N01}}}] (input) at (0,0) {\textbf{Input}\\\(P_{1\to5}\): linear local-site family \(\mathcal S\)};
\node[bcwide,bclemma,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N02}}}] (tests) at (0,-2.25) {\textbf{Lemma}\\site dichotomy: active or inactive in the branch state};
\node[bcwide,bclemma,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N03}}}] (demands) at (0,-4.70) {\textbf{Lemma}\\inactive sites create bounded local witnesses};
\node[bcauditbox,text width=3.75cm,minimum width=3.75cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N04}}}] (audit) at (0,-7.15) {\textbf{Lemma}\\each global estimate is backed by local witnesses?};
\node[bcroutechoice,bctheorem,text width=2.55cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N05}}}] (auditroute) at (-6.85,-7.15) {\textbf{Routing theorem}\\what failed in the locality lemma?};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N06}}}] (bad) at (-12.20,-7.15) {\textbf{Scope candidate}\\no bounded witnesses\\\scoperef};
\node[bcbox,bclemma,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N07}}}] (charge) at (0,-9.70) {\textbf{Lemma}\\canonical charging of witnesses to scarce payers};
\node[bcbox,bcpayload,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N08}}}] (nonlocalres) at (-6.85,-10.05) {\textbf{Payload record}\\\(P_{5\to11}\): additive global deficit};
\node[bcbox,bclemma,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N09}}}] (sum) at (0,-12.20) {\textbf{Lemma}\\local-to-global summation};
\node[bcbox,bclemma,bcpayload,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N10}}}] (loclemma) at (-6.85,-12.85) {\textbf{Lemma}\\\(P_{5\to11}\) satisfies CT11 trigger};
\node[bcdec,bctheorem,text width=3.05cm,minimum width=3.95cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N11}}}] (capacity) at (0,-15.25) {\textbf{Comparison theorem}\\does forced local mass exceed capacity?};
\node[bcbox,bclemma,bcpayload,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N12}}}] (chargemap) at (5.95,-15.25) {\textbf{Payload lemma}\\build \(P_{5\to4}\)\\\(\langle\mathcal D_{\mathcal S},\mathcal P_{\mathcal S},\pi_{\mathcal S},C_{\mathcal S},\ell_{\mathcal S}\rangle\)};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N13}}}] (chargeroute) at (10.70,-15.25) {\textbf{CT 4}\\charging};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N14}}}] (locroute) at (-6.85,-15.85) {\textbf{CT 11}\\localization};
\node[bcclosed,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N15}}}] (out) at (10.70,-11.95) {\textbf{C4}\\\(P_{5\to C4}\)\\global contradiction};
\node[bcbox,bcpayload,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N16}}}] (shortfall) at (0,-18.25) {\textbf{Payload record}\\\(P_{5\to14}\): local mass not yet decisive};
\node[bcbox,bclemma,bcpayload,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N17}}}] (masslemma) at (0,-20.75) {\textbf{Lemma}\\\(P_{5\to14}\) satisfies CT14 trigger};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT5.N18}}}] (massroute) at (0,-23.75) {\textbf{CT 14}\\aggregate closure};
...
\draw[bchandoff] (loclemma)--node[bclabel,right]{S-Trig for CT11}(locroute);
...
\draw[bchandoff] (chargemap)--node[bclabel,above]{S-Trig for CT4}(chargeroute);
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the activity threshold, activity predicate, first-failure order, and first-failure extractor.
\item \textbf{S-Dich} is instantiated with the active branch and the dormant branch produced by the monitored family.
\item \textbf{S-Equiv} is instantiated with the assertion that the first-failure witness carries target-relevant data.
\item \textbf{S-Rout} is instantiated with \(P_{6\to4}\), \(P_{6\to3}\), \(P_{6\to7}\), \(P_{6\to9}\), and \(P_{6\to10}\) for active demand, boundary response, exchange data, saturation data, and the refinement recovery residual.
\item \textbf{S-Trig} is instantiated with the CT4, CT3, CT7, CT9, and CT10 triggers for those active, dormant, or recovery outputs.
\end{itemize}
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N01}}}] (input) at (0,0) {\textbf{Definition}\\monitored local test and activity threshold};
\node[bcdec,bclemma,text width=3.05cm,minimum width=3.85cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N02}}}] (active) at (0,-2.90) {\textbf{Dichotomy lemma}\\are there enough active tests?};
\node[bcbox,bclemma,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N03}}}] (hot) at (6.20,-2.90) {\textbf{Lemma}\\active tests force explicit demand};
\node[bcbox,bclemma,bcpayload,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N04}}}] (hotout) at (6.20,-5.55) {\textbf{Payload lemma}\\build \(P_{6\to4}\)\\active demand ledger};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N05}}}] (hotroute) at (6.20,-8.55) {\textbf{CT 4}\\charging};
\node[bcbox,bcdefinition,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N06}}}] (cold) at (0,-5.55) {\textbf{Definition}\\dormant branch and first failing scale};
\node[bcbox,bclemma,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N07}}}] (witness) at (0,-8.15) {\textbf{Lemma}\\first-failure witness is bounded and structural};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N08}}}] (scopebad) at (-6.20,-8.15) {\textbf{Scope candidate}\\no bounded first failure\\\scoperef};
\node[bcbox,bcdesign,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N09}}}] (refineres) at (-10.85,-10.95) {\textbf{Payload record}\\\(P_{6\to10}\): witness appears after refinement};
\node[bcbox,bclemma,bcdesign,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N10}}}] (refinelemma) at (-10.85,-14.05) {\textbf{Lemma}\\\(P_{6\to10}\) satisfies CT10 trigger};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N11}}}] (refineroute) at (-10.85,-17.80) {\textbf{CT 10}\\refinement};
\node[bcroutechoice,bctheorem,text width=2.65cm,minimum width=3.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N12}}}] (coldroute) at (0,-11.30) {\textbf{Routing theorem}\\what witness type is certified?};
\node[bcbox,bclemma,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N13}}}] (hitlemma) at (-7.05,-14.65) {\textbf{Lemma}\\target-realization trigger};
\node[bcbox,bclemma,bcpayload,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N14}}}] (exlemma) at (-2.35,-14.65) {\textbf{Payload lemma}\\build \(P_{6\to7}\)\\same-interface witness};
\node[bcbox,bclemma,bcpayload,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N15}}}] (typelemma) at (2.35,-14.65) {\textbf{Payload lemma}\\build \(P_{6\to3}\)\\boundary-response witness};
\node[bcbox,bclemma,bcpayload,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N16}}}] (chargellemma) at (7.05,-14.65) {\textbf{Payload lemma}\\build \(P_{6\to9}\)\\saturated payer};
\node[bcclosed,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N17}}}] (hitout) at (-7.05,-17.80) {\textbf{C1}\\direct target hit};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N18}}}] (exchangefig) at (-2.35,-17.80) {\textbf{CT 7}\\exchange};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N19}}}] (typefig) at (2.35,-17.80) {\textbf{CT 3}\\external types};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT6.N20}}}] (chargefig) at (7.05,-17.80) {\textbf{CT 9}\\overload};
\draw[bcarr] (input)--(active);
\draw[bcarr] (active)--node[bclabel,pos=.43,above]{yes: active}(hot);
\draw[bcarr] (hot)--(hotout);
\draw[bchandoff] (hotout)--node[bclabel,right]{S-Trig for CT4}(hotroute);
...
\draw[bcrecoverarr] (refinelemma)--node[bclabel,right]{S-Trig for CT10}(refineroute);
...
\draw[bchandoff] (exlemma)--node[bclabel,right]{S-Trig for CT7}(exchangefig);
\draw[bchandoff] (typelemma)--node[bclabel,right]{S-Trig for CT3}(typefig);
\draw[bchandoff] (chargellemma)--node[bclabel,right]{S-Trig for CT9}(chargefig);
\end{tikzpicture}%
}
...
margin. The dormant mass feeds the charging machinery of Sections \secref{the-general-branch-closure-algorithm} and
...
\ctpar{Execution}
First verify S-Def for \((A,A',\iota,\delta)\): the objects share the
...
load measure.  On the separator-free branch, prove S-Equiv for generator
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the exchange pair \((A,A',\iota,\delta)\), the common interface, and the response evaluator on \(\iota\)-completions.
\item \textbf{S-Equiv} is instantiated with the completeness of the finite context generator \(\mathcal G(\iota)\) for all target-relevant compatible contexts.
\item \textbf{S-Dich} is instantiated with the realizing, distinguishing, and neutral alternatives for the bounded exchange.
\item \textbf{S-Comp} is instantiated with the compression comparison for a smaller neutral representative.
\item \textbf{S-Rout} is instantiated with \(P_{7\to3}\), \(P_{7\to12}\), \(P_{7\to10}\), and \(P_{7\to16}\) for external-type defects, peelable load, neutral labels, and whole-object data.
\item \textbf{S-Trig} is instantiated with the CT3, CT12, CT10, and CT16 triggers for those exchange outputs.
\end{itemize}
...
execution error is declaring neutrality with S-Equiv unfilled; the audit
records CT7/S-Equiv.
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N01}}}] (input) at (0,0) {\textbf{Definition}\\bounded exchange pair \(A\leftrightarrow A'\) with common interface};
\node[bcdec,bclemma,text width=3.15cm,minimum width=3.95cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N02}}}] (realize) at (0,-2.85) {\textbf{Lemma}\\does a compatible context realize \(T\)?};
\node[bcclosed,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N03}}}] (hit) at (6.90,-2.85) {\textbf{C1}\\direct target hit};
\node[bcdec,bclemma,text width=3.15cm,minimum width=3.95cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N04}}}] (distinguish) at (0,-6.15) {\textbf{Lemma}\\does a context distinguish \(A\) from \(A'\)?};
\node[bcroutechoice,bctheorem,text width=2.65cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N05}}}] (deflemma) at (7.45,-6.15) {\textbf{Routing theorem}\\which defect payload is certified?};
\node[bcclosed,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N06}}}] (routedclose) at (13.10,-6.15) {\textbf{C3}\\defect account already routed};
\node[bcbox,bclemma,bcpayload,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N07}}}] (extlemma) at (6.35,-9.20) {\textbf{Payload lemma}\\build \(P_{7\to3}\)\\external-type defect};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N08}}}] (defect) at (6.35,-12.05) {\textbf{CT 3}\\external types};
\node[bcbox,bclemma,bcpayload,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N09}}}] (peellemma) at (11.35,-9.20) {\textbf{Payload lemma}\\build \(P_{7\to12}\)\\load plus measure};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N10}}}] (peeldefect) at (11.35,-12.25) {\textbf{CT 12}\\peeling loop};
\node[bcdec,bclemma,text width=3.15cm,minimum width=3.95cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N11}}}] (neutral) at (0,-10.35) {\textbf{Lemma}\\same response in all contexts and smaller representative?};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N12}}}] (scopebad) at (-6.90,-10.35) {\textbf{Scope candidate}\\no finite context generator\\\scoperef};
\node[bcbox,bclemma,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N13}}}] (complemma) at (0,-13.65) {\textbf{Lemma}\\target-equivalent compression};
\node[bcclosed,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N14}}}] (compress) at (0,-16.65) {\textbf{C2}\\minimality contradiction};
\node[bcbox,bcdesign,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N15}}}] (neutralres) at (6.20,-14.20) {\textbf{Payload record}\\neutral noncompressive exchange};
\node[bcroutechoice,bctheorem,bcdesign,text width=2.55cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N16}}}] (neutralroute) at (6.20,-17.20) {\textbf{Routing theorem}\\what neutral datum remains?};
\node[bcbox,bclemma,bcdesign,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N17}}}] (reflemma) at (3.15,-20.30) {\textbf{Payload lemma}\\build \(P_{7\to10}\)\\neutral label};
\node[bcbox,bclemma,bcdesign,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N18}}}] (exactlemma) at (9.25,-20.30) {\textbf{Payload lemma}\\build \(P_{7\to16}\)\\whole-object data};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N19}}}] (refroute) at (3.15,-23.30) {\textbf{CT 10}\\refinement};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT7.N20}}}] (exactroute) at (9.25,-23.30) {\textbf{CT 16}\\whole-object types};
...
\draw[bchandoff] (extlemma)--node[bclabel,left]{S-Trig for CT3}(defect);
\draw[bcarr] (deflemma)--node[bclabel,right,text width=1.85cm,align=center]{routed load}(peellemma);
\draw[bchandoff] (peellemma)--node[bclabel,right]{S-Trig for CT12}(peeldefect);
...
\draw[bcrecoverarr] (reflemma)--node[bclabel,right]{S-Trig for CT10}(refroute);
\draw[bcrecoverarr] (exactlemma)--node[bclabel,right]{S-Trig for CT16}(exactroute);
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the bounded sequence \(S_0,\ldots,S_m\), the finite label or exact-type alphabet, and the equality test.
\item \textbf{S-Dich} is instantiated with the split between a repeated exact type and a short finite survivor list.
\item \textbf{S-Equiv} is instantiated with the claim that repeated types have equal response in all compatible contexts.
\item \textbf{S-Comp} is instantiated with the finite threshold, the finite enumeration, or the repeated-type compression comparison.
\item \textbf{S-Rout} is instantiated with \(P_{8\to3}\) for separating type data, \(P_{8\to7}\) for chain exchange data, and \(P_{8\to10}\) for a coarse finite label design.
\item \textbf{S-Trig} is instantiated with the CT3 trigger for separating types, the CT7 trigger for chain exchanges, and the CT10 trigger for finite label refinement.
\end{itemize}
...
error is informal periodicity; the audit records CT8/S-Equiv or CT8/S-Dich.
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N01}}}] (input) at (0,0) {\textbf{Definition}\\bounded-width sequence \(S_0,\ldots,S_m\)};
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N02}}}] (types) at (0,-2.20) {\textbf{Definition}\\finite label or exact external type \(\tau(S_i)\)};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N03}}}] (scopebad) at (6.20,-2.20) {\textbf{Scope candidate}\\no finite type space\\\scoperef};
\node[bcbox,bcdesign,bcpayload,text width=3.15cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N04}}}] (labelres) at (-6.20,-5.10) {\textbf{Payload record}\\\(P_{8\to10}\): label space too coarse};
\node[bcbox,bclemma,bcdesign,bcpayload,text width=3.15cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N05}}}] (labeldesign) at (-6.20,-8.25) {\textbf{Payload lemma}\\\(P_{8\to10}\) satisfies CT10 trigger};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.15cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N06}}}] (labelroute) at (-6.20,-11.40) {\textbf{CT 10}\\refinement};
\node[bcdec,bclemma,text width=3.10cm,minimum width=3.90cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N07}}}] (repeat) at (0,-5.10) {\textbf{Pigeonhole lemma}\\does an exact type repeat?};
\node[bcbox,bclemma,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N08}}}] (short) at (6.20,-5.10) {\textbf{Lemma}\\finite-threshold certificate};
\node[bcclosed,text width=3.15cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N09}}}] (shortroute) at (6.20,-8.15) {\textbf{C5}\\finite enumeration};
\node[bcdec,bclemma,text width=3.10cm,minimum width=3.90cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N10}}}] (equal) at (0,-8.25) {\textbf{Dichotomy lemma}\\are repeated states equal in all contexts?};
\node[bcbox,bclemma,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N11}}}] (defect) at (6.20,-11.25) {\textbf{Lemma}\\separating-context trigger};
\node[bcroutechoice,bctheorem,text width=2.55cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N12}}}] (defectroute) at (6.20,-14.10) {\textbf{Routing theorem}\\how is the witness used?};
\node[bcbox,bclemma,bcpayload,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N13}}}] (typelemma) at (2.45,-17.25) {\textbf{Payload lemma}\\build \(P_{8\to3}\)\\separating type};
\node[bcbox,bclemma,bcpayload,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N14}}}] (exlemma) at (8.05,-17.25) {\textbf{Payload lemma}\\build \(P_{8\to7}\)\\chain exchange};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N15}}}] (typeroute) at (2.45,-20.25) {\textbf{CT 3}\\external types};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N16}}}] (exroute) at (8.05,-20.25) {\textbf{CT 7}\\exchange};
\node[bcbox,bclemma,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N17}}}] (complemma) at (0,-11.25) {\textbf{Lemma}\\repeated-type compression};
\node[bcclosed,text width=3.15cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT8.N18}}}] (compress) at (0,-14.25) {\textbf{C2}\\minimality contradiction};
...
\draw[bcrecoverarr] (labeldesign)--node[bclabel,right]{S-Trig for CT10}(labelroute);
...
\draw[bchandoff] (typelemma)--node[bclabel,right]{S-Trig for CT3}(typeroute);
\draw[bchandoff] (exlemma)--node[bclabel,right]{S-Trig for CT7}(exroute);
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the payer \(p\), the fibre \(F_p=\pi^{-1}(p)\), the finite label map on the fibre, and the capacity bound.
\item \textbf{S-Dich} is instantiated with the bounded-fibre branch and the overloaded-fibre branch.
\item \textbf{S-Comp} is instantiated with the pigeonhole or homogeneous-extraction comparison used to prove bounded multiplicity.
\item \textbf{S-Rout} is instantiated with \(P_{9\to4}\), \(P_{9\to7}\), \(P_{9\to8}\), and \(P_{9\to10}\) for bounded multiplicity, homogeneous exchange, long-chain data, and finite-label recovery.
\item \textbf{S-Trig} is instantiated with the CT4, CT7, CT8, and CT10 triggers for those overload or recovery outputs.
\end{itemize}
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N01}}}] (input) at (0,0) {\textbf{Definition}\\charging map \(\pi:\mathcal D\to\mathcal P\)};
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N02}}}] (fiberdef) at (0,-2.15) {\textbf{Definition}\\payer fibre \(F_p=\pi^{-1}(p)\)};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N03}}}] (scopebad) at (6.20,-2.15) {\textbf{Scope candidate}\\no finite labelling\\\scoperef};
\node[bcbox,bcdesign,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N04}}}] (labelres) at (-6.20,-5.05) {\textbf{Payload record}\\\(P_{9\to10}\): label refinement needed};
\node[bcbox,bclemma,bcdesign,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N05}}}] (labellemma) at (-6.20,-8.10) {\textbf{Lemma}\\\(P_{9\to10}\) satisfies CT10 trigger};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N06}}}] (labelroute) at (-6.20,-11.15) {\textbf{CT 10}\\refinement};
\node[bcdec,bclemma,text width=3.15cm,minimum width=4.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N07}}}] (fiber) at (0,-5.05) {\textbf{Dichotomy lemma}\\is some fibre overloaded?};
\node[bcbox,bclemma,bcpayload,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N08}}}] (bounded) at (6.20,-5.05) {\textbf{Payload lemma}\\build \(P_{9\to4}\)\\bounded multiplicity};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N09}}}] (boundedroute) at (6.20,-8.10) {\textbf{CT 4}\\charging};
\node[bcbox,bcdefinition,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N10}}}] (overload) at (0,-8.10) {\textbf{Branch-state definition}\\overloaded labelled fibre};
\node[bcbox,bclemma,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N11}}}] (mono) at (0,-10.70) {\textbf{Pigeonhole lemma}\\homogeneous overloaded subfamily};
\node[bcbox,bclemma,text width=3.45cm,minimum width=3.45cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N12}}}] (extract) at (0,-13.30) {\textbf{Instantiation lemma}\\target hit, same-interface exchange, or long chain};
\node[bcroutechoice,bctheorem,text width=2.70cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N13}}}] (route) at (0,-16.45) {\textbf{Routing theorem}\\what object was extracted?};
\node[bcbox,bclemma,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N14}}}] (hitlemma) at (-5.90,-19.80) {\textbf{Lemma}\\target-hit trigger};
\node[bcbox,bclemma,bcpayload,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N15}}}] (exchangelemma) at (0,-19.80) {\textbf{Payload lemma}\\build \(P_{9\to7}\)\\same-interface object};
\node[bcbox,bclemma,bcpayload,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N16}}}] (pumplemma) at (5.90,-19.80) {\textbf{Payload lemma}\\build \(P_{9\to8}\)\\long chain};
\node[bcclosed,text width=2.95cm,minimum width=2.95cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N17}}}] (hitout) at (-5.90,-22.85) {\textbf{C1}\\direct target hit};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N18}}}] (exchangeout) at (0,-22.85) {\textbf{CT 7}\\exchange};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT9.N19}}}] (pumpout) at (5.90,-22.85) {\textbf{CT 8}\\pumping};
...
\draw[bcrecoverarr] (labellemma)--node[bclabel,right]{S-Trig for CT10}(labelroute);
\draw[bcarr] (fiberdef)--(fiber);
\draw[bcarr] (fiber)--node[bclabel,above]{no: all fibres bounded}(bounded);
\draw[bchandoff] (bounded)--node[bclabel,right]{S-Trig for CT4}(boundedroute);
...
\draw[bcarr] (hitlemma)--(hitout);
\draw[bchandoff] (exchangelemma)--node[bclabel,right]{S-Trig for CT7}(exchangeout);
\draw[bchandoff] (pumplemma)--node[bclabel,right]{S-Trig for CT8}(pumpout);
\end{tikzpicture}%
}
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the residual class, the promoted finite datum, the refined alphabet \(\Lambda'\), and the class table.
\item \textbf{S-Dich} is instantiated with the refined label-class alternatives produced by \(\Lambda'\).
\item \textbf{S-Comp} is instantiated with the finite class-exhaustion check inside each refined label class.
\item \textbf{S-Rout} is instantiated with \(P_{10\to3}\), \(P_{10\to7}\), and \(P_{10\to15}\) for promoted response data, class exchanges, and rank data.
\item \textbf{S-Trig} is instantiated with the CT3, CT7, and CT15 triggers for the promoted payloads.
\end{itemize}
...
the audit records CT10/S-Def or CT10/S-Rout.
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N01}}}] (input) at (0,0) {\textbf{Definition}\\surviving residual class \(\mathcal C\)};
\node[bcbox,bcdesign,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N02}}}] (recoverentry) at (-6.35,0) {\textbf{Recovery entry}\\payloads from CT2, CT6, CT7, CT8, CT9, CT11, CT14, CT17};
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N03}}}] (label) at (0,-2.25) {\textbf{Definition}\\finite refinement label \(\ell:\mathcal C\to\Lambda\)};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N04}}}] (scopebad) at (6.35,-2.25) {\textbf{Scope candidate}\\infinite refinement alphabet\\\scoperef};
\node[bcbox,bclemma,bccheck,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N05}}}] (exhaust) at (0,-4.70) {\textbf{Lemma}\\label classes exhaust \(\mathcal C\)};
\node[bcbox,bclemma,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N06}}}] (exchange) at (0,-7.20) {\textbf{Lemma}\\supported exchanges are finite inside each label class};
\node[bcroutechoice,bctheorem,text width=2.70cm,minimum width=3.40cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N07}}}] (outcome) at (0,-10.35) {\textbf{Classification theorem}\\what happens in one label class?};
\node[bcbox,bcproposition,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N08}}}] (closeprop) at (-6.35,-13.80) {\textbf{Proposition}\\label class is empty};
\node[bcclosed,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N09}}}] (classclose) at (-6.35,-16.90) {\textbf{C5}\\label class closed};
\node[bcbox,bclemma,bcpayload,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N10}}}] (routelemma) at (0,-13.80) {\textbf{Payload lemma}\\class exchange or response/type datum};
\node[bcroutechoice,bctheorem,text width=2.55cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N11}}}] (closeroute) at (0,-16.90) {\textbf{Routing theorem}\\class result};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N12}}}] (exchangefig) at (-2.75,-20.10) {\textbf{CT 7}\\exchange};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N13}}}] (typefig) at (2.55,-23.30) {\textbf{CT 3}\\external types};
\node[bcbox,bclemma,bcpayload,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N14}}}] (promote) at (6.35,-13.80) {\textbf{Lemma}\\missing-datum extraction};
\node[bcbox,bcdefinition,bcpayload,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N15}}}] (next) at (6.35,-16.90) {\textbf{Definition}\\promoted invariant};
\node[bcroutechoice,bctheorem,text width=2.55cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N16}}}] (nextfig) at (6.35,-20.10) {\textbf{Routing theorem}\\new invariant type};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT10.N17}}}] (rankgen) at (9.30,-23.30) {\textbf{CT 15}\\rank forcing};
...
\draw[bchandoff] (closeroute)--node[bclabel,above]{S-Trig for CT7}(exchangefig);
\draw[bchandoff] (closeroute) to[bend left=10] node[bclabel,left,pos=.72]{S-Trig for CT3}(typefig);
\draw[bcarr] (outcome)--node[bclabel,above]{case: missing datum}(promote);
\draw[bcarr] (promote)--(next);
\draw[bcarr] (next)--(nextfig);
\draw[bchandoff] (nextfig) to[bend left=10] node[bclabel,above,sloped,pos=.58]{S-Trig for CT3}(typefig);
\draw[bchandoff] (nextfig)--node[bclabel,right,pos=.55]{S-Trig for CT15}(rankgen);
\end{tikzpicture}%
}
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the additive budget, the admissible decomposition, the admissibility class, and the localized piece data.
\item \textbf{S-Comp} is instantiated with the summation and averaging comparison that produces a connected local piece with negative budget.
\item \textbf{S-Rout} is instantiated with \(P_{11\to1}\), \(P_{11\to7}\), \(P_{11\to10}\), and \(P_{11\to14}\) for target-test data, localized exchange data, admissibility gaps, and carried mass.
\item \textbf{S-Trig} is instantiated with the CT1, CT7, CT10, and CT14 triggers for those localized outputs.
\end{itemize}
...
proved; the audit records CT11/S-Def.
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N01}}}] (input) at (0,0) {\textbf{Definition}\\additive budget \(B(R)<0\) on residual region \(R\)};
\node[bcwide,bclemma,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N02}}}] (decomp) at (0,-2.25) {\textbf{Lemma}\\admissible decomposition into connected pieces};
\node[bcauditbox,text width=3.75cm,minimum width=3.75cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N03}}}] (closed) at (0,-4.70) {\textbf{Lemma}\\admissibility is preserved by local moves?};
\node[bcroutechoice,bctheorem,bcdesign,text width=2.55cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N04}}}] (toolroute) at (6.70,-4.70) {\textbf{Routing theorem}\\why did admissibility fail?};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N05}}}] (bad) at (11.70,-4.70) {\textbf{Scope candidate}\\no finite local type\\\scoperef};
\node[bcbox,bclemma,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N06}}}] (sum) at (0,-7.20) {\textbf{Lemma}\\summation of local contributions};
\node[bcbox,bcdesign,text width=3.30cm,minimum width=3.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N07}}}] (toolres) at (6.70,-7.20) {\textbf{Payload record}\\\(P_{11\to10}\): admissibility gap};
\node[bcdec,bclemma,text width=3.10cm,minimum width=3.95cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N08}}}] (local) at (0,-10.25) {\textbf{Localization lemma}\\does some connected \(X\subseteq R\) have \(B(X)<0\)?};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N09}}}] (badlocal) at (-6.00,-10.25) {\textbf{Admission reject}\\additivity not certified};
\node[bcbox,bclemma,bcdesign,text width=3.30cm,minimum width=3.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N10}}}] (toollemma) at (6.70,-10.25) {\textbf{Lemma}\\\(P_{11\to10}\) satisfies CT10 trigger};
\node[bcbox,bcdefinition,bcpayload,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N11}}}] (object) at (0,-13.40) {\textbf{Definition}\\localized deficit piece \(X\)};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N12}}}] (toolfix) at (6.70,-13.40) {\textbf{CT 10}\\refinement};
\node[bcroutechoice,bctheorem,text width=2.70cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N13}}}] (nextroute) at (0,-16.55) {\textbf{Routing theorem}\\which local object was found?};
\node[bcbox,bclemma,bcpayload,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N14}}}] (targetlemma) at (-6.00,-19.90) {\textbf{Payload lemma}\\build \(P_{11\to1}\)\\target-test data};
\node[bcbox,bclemma,bcpayload,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N15}}}] (exchangelemma) at (0,-19.90) {\textbf{Payload lemma}\\build \(P_{11\to7}\)\\localized exchange};
\node[bcbox,bclemma,bcpayload,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N16}}}] (aggregatelemma) at (6.00,-19.90) {\textbf{Payload lemma}\\build \(P_{11\to14}\)\\carried mass};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N17}}}] (targetroute) at (-6.00,-22.95) {\textbf{CT 1}\\local target tests};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N18}}}] (exchangeroute) at (0,-22.95) {\textbf{CT 7}\\exchange};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT11.N19}}}] (aggregateroute) at (6.00,-22.95) {\textbf{CT 14}\\aggregate closure};
...
\draw[bcrecoverarr] (toollemma)--node[bclabel,right]{S-Trig for CT10}(toolfix);
...
\draw[bchandoff] (targetlemma)--node[bclabel,right]{S-Trig for CT1}(targetroute);
\draw[bchandoff] (exchangelemma)--node[bclabel,right]{S-Trig for CT7}(exchangeroute);
\draw[bchandoff] (aggregatelemma)--node[bclabel,right]{S-Trig for CT14}(aggregateroute);
\end{tikzpicture}%
}
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the routed load account, the peel move, and the measure \(L\).
...
\item \textbf{S-Comp} is instantiated with the termination or discharge comparison that closes the unsaturated branch.
\item \textbf{S-Rout} is instantiated with \(P_{12\to4}\) and \(P_{12\to13}\) for ordinary charge demand and tier-interaction demand.
\item \textbf{S-Trig} is instantiated with the CT4 and CT13 triggers for demand created by a peel pass.
\end{itemize}
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N01}}}] (input) at (0,0) {\textbf{Definition}\\saturated branch state with routed load \(L\)};
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N02}}}] (measuredef) at (0,-2.15) {\textbf{Definition}\\well-founded measure \(M(L)\in\mathbb N\)};
\node[bcdec,bclemma,text width=3.05cm,minimum width=3.85cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N03}}}] (sat) at (0,-5.05) {\textbf{Dichotomy lemma}\\is a peeling exit triggered?};
\node[bcbox,bclemma,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N04}}}] (discharge) at (6.20,-5.05) {\textbf{Lemma}\\unsaturated discharge inequality};
\node[bcclosed,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N05}}}] (unsat) at (6.20,-8.10) {\textbf{C4}\\unsaturated branch};
\node[bcbox,bclemma,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N06}}}] (peel) at (0,-8.10) {\textbf{Lemma}\\per-pass peel removes one routed unit};
\node[bcauditbox,text width=3.65cm,minimum width=3.65cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N07}}}] (restore) at (0,-10.65) {\textbf{Lemma}\\branch state restored and new demands paid?};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N08}}}] (scopebad) at (-6.70,-2.15) {\textbf{Scope candidate}\\load not measurable\\\scoperef};
\node[bcroutechoice,bctheorem,bcdesign,text width=2.55cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N09}}}] (restorefail) at (6.20,-10.65) {\textbf{Routing theorem}\\why did restoration fail?};
\node[bcbox,bclemma,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N10}}}] (measure) at (0,-13.20) {\textbf{Lemma}\\strict decrease \(M(L')<M(L)\)};
\node[bcbox,bcdesign,text width=3.30cm,minimum width=3.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N11}}}] (statefix) at (6.20,-13.35) {\textbf{Payload record}\\restored state with new demand};
\node[bcroutechoice,bctheorem,bcdesign,text width=2.55cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N12}}}] (repairroute) at (6.20,-16.35) {\textbf{Routing theorem}\\what repair data is present?};
\node[bcbox,bclemma,bcdesign,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N13}}}] (chargelemma) at (3.45,-19.35) {\textbf{Payload lemma}\\build \(P_{12\to4}\)\\ordinary demand};
\node[bcbox,bclemma,bcdesign,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N14}}}] (tierlemma) at (8.95,-19.35) {\textbf{Payload lemma}\\build \(P_{12\to13}\)\\tier interaction};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N15}}}] (chargeroute) at (3.45,-22.35) {\textbf{CT 4}\\charging};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT12.N16}}}] (tierroute) at (8.95,-22.35) {\textbf{CT 13}\\tiered charging};
...
\draw[bcrecoverarr] (chargelemma)--node[bclabel,right]{S-Trig for CT4}(chargeroute);
\draw[bcrecoverarr] (tierlemma)--node[bclabel,right]{S-Trig for CT13}(tierroute);
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the tier-one account, the fallback tier data, and the availability hypothesis.
...
\item \textbf{S-Dich} is instantiated with the alternatives that the tier-one scheme is available or a fallback obstruction must be charged.
\item \textbf{S-Comp} is instantiated with the reconciliation lemma and the combined tier-capacity comparison.
\item \textbf{S-Rout} is instantiated with \(P_{13\to4}\), \(P_{13\to9}\), and \(P_{13\to14}\) for tier-one accounts, overlap fibres, and overlap mass.
\item \textbf{S-Trig} is instantiated with the CT4, CT9, and CT14 triggers for those tiered-charging outputs.
\end{itemize}
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N01}}}] (input) at (0,0) {\textbf{Definition}\\demands \(\mathcal D\), tier-1 payers \(\mathcal P_1\), and availability hypothesis \(A\)};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N02}}}] (scopebad) at (-5.75,-2.95) {\textbf{Scope candidate}\\no measurable tier resource\\\scoperef};
\node[bcdec,bclemma,text width=3.05cm,minimum width=3.85cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N03}}}] (avail) at (0,-2.95) {\textbf{Dichotomy lemma}\\is \(A\) available?};
\node[bcbox,bclemma,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N04}}}] (tierone) at (5.75,-2.95) {\textbf{Lemma}\\tier-1 canonical payment};
\node[bcbox,bclemma,bcpayload,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N05}}}] (capone) at (5.75,-5.55) {\textbf{Payload lemma}\\build \(P_{13\to4}\)\\tier-1 account};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N06}}}] (caproute) at (5.75,-8.55) {\textbf{CT 4}\\charging};
\node[bcbox,bclemma,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N07}}}] (minobs) at (0,-5.55) {\textbf{Lemma}\\minimal obstruction \(O\) to availability};
\node[bcbox,bclemma,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N08}}}] (tiertwo) at (0,-8.15) {\textbf{Lemma}\\tier-2 charge to fallback payers \(\mathcal P_2\)};
\node[bcauditbox,text width=3.65cm,minimum width=3.65cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N09}}}] (reconcile) at (0,-10.75) {\textbf{Lemma}\\tier resources disjoint or explicitly reconciled?};
\node[bcroutechoice,bctheorem,text width=2.55cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N10}}}] (overlaproute) at (7.25,-10.75) {\textbf{Routing theorem}\\what remains after reconciliation?};
\node[bcbox,bctheorem,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N11}}}] (combined) at (0,-13.35) {\textbf{Theorem}\\combined tier capacity bound};
\node[bcbox,bcpayload,text width=3.30cm,minimum width=3.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N12}}}] (overlapres) at (7.25,-13.35) {\textbf{Payload record}\\overlap-obstruction residual};
\node[bcbox,bclemma,bcpayload,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N13}}}] (overlaplemma) at (7.25,-15.95) {\textbf{Payload lemma}\\build \(P_{13\to9}\) or \(P_{13\to14}\)};
\node[bcclosed,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N14}}}] (captwo) at (0,-16.35) {\textbf{C4}\\capacity contradiction};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N15}}}] (overroute) at (4.50,-18.95) {\textbf{CT 9}\\overload};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT13.N16}}}] (massroute) at (10.00,-18.95) {\textbf{CT 14}\\aggregate closure};
...
\draw[bchandoff] (capone)--node[bclabel,right]{S-Trig for CT4}(caproute);
...
\draw[bchandoff] (overlaplemma)--node[bclabel,right]{S-Trig for CT9}(overroute);
\draw[bchandoff] (overlaplemma)--node[bclabel,right]{S-Trig for CT14}(massroute);
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the residual class \(\mathcal C\), its recorded branch state, and the mass and capacity records attached to the class.
\item \textbf{S-Comp} is instantiated with the lower-mass estimate, upper-capacity estimate, carried-charge multiplicity bound, and final aggregate contradiction.
\item \textbf{S-Rout} is instantiated with \(P_{14\to9}\) for a named unbounded-capacity member or multiplicity gap and \(P_{14\to10}\) for a bounded missing capacity-classification datum.
\item \textbf{S-Trig} is instantiated with the CT9 trigger for the unbounded-member overload payload and the CT10 trigger for finite capacity-label refinement.
\end{itemize}
...
multiplicity; the audit records CT14/S-Comp.
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N01}}}] (input) at (0,0) {\textbf{Definition}\\residual class \(\mathcal C\) with recorded branch state};
\node[bcbox,bclemma,text width=3.65cm,minimum width=3.65cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N02}}}] (lower) at (-4.35,-3.00) {\textbf{Lemma}\\branch state forces \(\mathcal C\) to carry \(\ge cn\) deficit};
\node[bcbox,bclemma,text width=3.65cm,minimum width=3.65cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N03}}}] (upper) at (4.35,-3.00) {\textbf{Lemma}\\members of \(\mathcal C\) charge to a global \(o(n)\) resource};
\node[bcauditbox,text width=3.85cm,minimum width=3.85cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N04}}}] (mult) at (0,-6.00) {\textbf{Lemma}\\upper bound counts carried-charge multiplicity?};
\node[bcroutechoice,bctheorem,bcdesign,text width=2.55cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N05}}}] (multroute) at (6.35,-6.00) {\textbf{Routing theorem}\\which capacity datum failed?};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N06}}}] (scopebad) at (12.40,-6.00) {\textbf{Scope candidate}\\no finite bounded-capacity description\\\scoperef};
\node[bcbox,bctheorem,text width=3.65cm,minimum width=3.65cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N07}}}] (compare) at (0,-8.70) {\textbf{Comparison theorem}\\\(\ge cn\) demand versus \(o(n)\) capacity};
\node[bcbox,bcdesign,bcpayload,text width=3.30cm,minimum width=3.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N08}}}] (unbounded) at (4.20,-8.70) {\textbf{Payload record}\\\(P_{14\to9}\): unbounded-capacity member};
\node[bcbox,bcdesign,bcpayload,text width=3.30cm,minimum width=3.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N09}}}] (refine) at (8.50,-8.70) {\textbf{Payload record}\\\(P_{14\to10}\): missing capacity label};
\node[bcbox,bclemma,bcdesign,bcpayload,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N10}}}] (overlemma) at (4.20,-11.35) {\textbf{Payload lemma}\\\(P_{14\to9}\) satisfies CT9 trigger};
\node[bcbox,bclemma,bcdesign,bcpayload,text width=3.20cm,minimum width=3.20cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N11}}}] (refinelemma) at (8.50,-11.35) {\textbf{Payload lemma}\\\(P_{14\to10}\) satisfies CT10 trigger};
\node[bcclosed,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N12}}}] (closed) at (0,-11.90) {\textbf{C4}\\\(\mathcal C\) is empty on the surviving branch};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N13}}}] (overroute) at (4.20,-14.25) {\textbf{CT 9}\\overload};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT14.N14}}}] (refineroute) at (8.50,-14.25) {\textbf{CT 10}\\refinement};
...
\draw[bcrecoverarr] (overlemma)--node[bclabel,right]{S-Trig for CT9}(overroute);
\draw[bcrecoverarr] (refinelemma)--node[bclabel,right]{S-Trig for CT10}(refineroute);
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the target-relative rank map, the dependence classifier, and the full-rank ledger.
\item \textbf{S-Dich} is instantiated with the rank-drop branch and the full-rank branch.
\item \textbf{S-Comp} is instantiated with the rank-budget comparison on certified independent coordinates.
\item \textbf{S-Rout} is instantiated with \(P_{15\to3}\), \(P_{15\to4}\), \(P_{15\to7}\), and \(P_{15\to16}\) for rank defects, full-rank ledgers, rank exchanges, and rank delocalization.
\item \textbf{S-Trig} is instantiated with the CT3, CT4, CT7, and CT16 triggers for those rank-forcing outputs.
\end{itemize}
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N01}}}] (input) at (0,0) {\textbf{Definition}\\raw local tests with target-relevant rank \(r_T\)};
\node[bcbox,bcdesign,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N02}}}] (rankrepair) at (-5.95,-2.20) {\textbf{Repair datum}\\rank definition not target-relative};
\node[bcbox,bclemma,bcdesign,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N03}}}] (rankrepairlemma) at (-5.95,-5.05) {\textbf{Lemma}\\redesign rank relative to \(T\)};
\node[bcauditbox,text width=3.70cm,minimum width=3.70cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N04}}}] (targetaudit) at (0,-2.20) {\textbf{Lemma}\\rank is defined relative to the target predicate?};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N05}}}] (scopebad) at (5.95,-2.20) {\textbf{Scope candidate}\\no finite rank\\\scoperef};
\node[bcdec,bclemma,text width=3.05cm,minimum width=3.85cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N06}}}] (rank) at (0,-5.05) {\textbf{Dichotomy lemma}\\does a rank drop occur?};
\node[bcbox,bclemma,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N07}}}] (dep) at (-5.95,-8.15) {\textbf{Lemma}\\rank drop produces structural dependence};
\node[bcroutechoice,bctheorem,text width=2.65cm,minimum width=3.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N08}}}] (structroute) at (-5.95,-11.30) {\textbf{Routing theorem}\\what dependence is certified?};
\node[bcbox,bclemma,bcpayload,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N09}}}] (typelemma) at (-10.00,-14.60) {\textbf{Payload lemma}\\build \(P_{15\to3}\)\\rank defect};
\node[bcbox,bclemma,bcpayload,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N10}}}] (exchangelemma) at (-5.95,-14.60) {\textbf{Payload lemma}\\build \(P_{15\to7}\)\\rank exchange};
\node[bcbox,bclemma,bcpayload,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N11}}}] (wholelemma) at (-1.90,-14.60) {\textbf{Payload lemma}\\build \(P_{15\to16}\)\\rank delocalization};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=2.95cm,minimum width=2.95cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N12}}}] (typeroute) at (-10.00,-17.65) {\textbf{CT 3}\\external types};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=2.95cm,minimum width=2.95cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N13}}}] (exchangeroute) at (-5.95,-17.65) {\textbf{CT 7}\\exchange};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=2.95cm,minimum width=2.95cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N14}}}] (wholeroute) at (-1.90,-17.65) {\textbf{CT 16}\\whole-object types};
\node[bcbox,bclemma,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N15}}}] (full) at (5.95,-8.15) {\textbf{Lemma}\\full-rank branch gives independent coordinates};
\node[bcbox,bclemma,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N16}}}] (charge) at (5.95,-10.75) {\textbf{Lemma}\\recorded rank charge};
\node[bcdec,bctheorem,text width=3.05cm,minimum width=3.85cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N17}}}] (compare) at (5.95,-13.95) {\textbf{Comparison theorem}\\does rank demand exceed cap?};
\node[bcclosed,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N18}}}] (budget) at (11.75,-13.95) {\textbf{C4}\\rank budget contradiction};
\node[bcbox,bcpayload,text width=3.30cm,minimum width=3.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N19}}}] (ledger) at (5.95,-17.25) {\textbf{Payload record}\\\(P_{15\to4}\): rank demand ledger};
\node[bcbox,bclemma,bcpayload,text width=3.30cm,minimum width=3.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N20}}}] (chargelemma) at (5.95,-19.75) {\textbf{Lemma}\\\(P_{15\to4}\) satisfies CT4 trigger};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT15.N21}}}] (chargeroute) at (5.95,-22.75) {\textbf{CT 4}\\charging};
...
\draw[bchandoff] (typelemma)--node[bclabel,left]{S-Trig for CT3}(typeroute);
\draw[bchandoff] (exchangelemma)--node[bclabel,right]{S-Trig for CT7}(exchangeroute);
\draw[bchandoff] (wholelemma)--node[bclabel,right]{S-Trig for CT16}(wholeroute);
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the delocalized whole-object datum, the computable closed-type slot, and the literal equality test.
\item \textbf{S-Equiv} is instantiated with closed response equality and the no-silent-identification rule for the empty-context case.
\item \textbf{S-Dich} is instantiated with the split between proper support and whole-object support.
\item \textbf{S-Rout} is instantiated with \(P_{16\to3}\) for proper-piece context data and \(P_{16\to10}\) for distinct closed data.
\item \textbf{S-Trig} is instantiated with the CT3 and CT10 triggers for the whole-object exits.
\end{itemize}
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT16.N01}}}] (input) at (0,0) {\textbf{Definition}\\delocalized distinguishing structure \(X\subseteq Z\)};
\node[bcdec,bclemma,text width=3.05cm,minimum width=3.85cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT16.N02}}}] (whole) at (0,-2.95) {\textbf{Dichotomy lemma}\\is \(Z=G\)?};
\node[bcbox,bclemma,bcpayload,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT16.N03}}}] (proper) at (5.80,-2.95) {\textbf{Payload lemma}\\build \(P_{16\to3}\)\\proper-piece context};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT16.N04}}}] (replace) at (5.80,-5.95) {\textbf{CT 3}\\external types};
\node[bcbox,bcdefinition,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT16.N05}}}] (closedtype) at (0,-5.95) {\textbf{Definition}\\exact closed type for the whole object};
\node[bcbox,bclemma,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT16.N06}}}] (compute) at (0,-8.55) {\textbf{Lemma}\\closed-type computation};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT16.N07}}}] (scopebad) at (5.80,-8.55) {\textbf{Scope candidate}\\closed type not finite\\\scoperef};
\node[bcdec,bclemma,bccheck,text width=3.05cm,minimum width=3.85cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT16.N08}}}] (equal) at (0,-11.70) {\textbf{Lemma}\\are the closed data literally equal?};
\node[bcbox,bcpayload,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT16.N09}}}] (distinct) at (-5.80,-11.70) {\textbf{Payload record}\\\(P_{16\to10}\): distinct closed type};
\node[bcbox,bclemma,bcpayload,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT16.N10}}}] (defaultlemma) at (-5.80,-14.55) {\textbf{Lemma}\\\(P_{16\to10}\) satisfies CT10 trigger};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.25cm,minimum width=3.25cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT16.N11}}}] (defaultroute) at (-5.80,-17.55) {\textbf{CT 10}\\refinement};
\node[bcbox,bclemma,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT16.N12}}}] (idlemma) at (0,-14.55) {\textbf{Lemma}\\exact-type identification};
\node[bcclosed,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT16.N13}}}] (out) at (0,-17.55) {\textbf{C2}\\whole-object identification};
\draw[bcarr] (input)--(whole);
\draw[bcarr] (whole)--node[bclabel,above]{proper piece}(proper);
\draw[bchandoff] (proper)--node[bclabel,right]{S-Trig for CT3}(replace);
...
\draw[bchandoff] (defaultlemma)--node[bclabel,right]{S-Trig for CT10}(defaultroute);
...
\begin{itemize}
\tightlist
\item \textbf{S-Def} is instantiated with the sparse target set \(S\), the bounded offset family \(\Delta\), the completion classes \(\Omega\), and the target arithmetic data.
\item \textbf{S-Dich} is instantiated with the finite-range branch and the repeated-increment branch of the scale split.
\item \textbf{S-Comp} is instantiated with the finite survivor enumeration or the large-scale arithmetic comparison.
\item \textbf{S-Rout} is instantiated with \(P_{17\to3}\), \(P_{17\to10}\), \(P_{17\to8}\), and \(P_{17\to14}\) for type separation, label separation, finite survivor sequences, and residual mass.
\item \textbf{S-Trig} is instantiated with the CT3, CT10, CT8, and CT14 triggers for those target-thickening outputs.
\end{itemize}
...
class record; the audit records CT17/S-Def.
...
\node[bcwide,bcdefinition,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N01}}}] (input) at (0,0) {\textbf{Definition}\\sparse target set and bounded local offset family};
\node[bcauditbox,text width=3.75cm,minimum width=3.75cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N02}}}] (compatible) at (0,-2.25) {\textbf{Lemma}\\offsets are realizable in the same branch state?};
\node[bcscopefail,text width=3.05cm,minimum width=3.05cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N03}}}] (scopebad) at (-6.60,-2.25) {\textbf{Scope candidate}\\no bounded offsets\\\scoperef};
\node[bcbox,bclemma,text width=3.55cm,minimum width=3.55cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N04}}}] (block) at (0,-4.85) {\textbf{Lemma}\\one structural event yields the translate \(x+\Delta\)};
\node[bcbox,bcdesign,text width=3.30cm,minimum width=3.30cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N05}}}] (compres) at (9.35,-2.25) {\textbf{Payload record}\\incompatible completion classes};
\node[bcdec,bctheorem,text width=3.05cm,minimum width=3.85cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N06}}}] (scale) at (0,-7.95) {\textbf{Scale-split theorem}\\finite range or repeated increments?};
\node[bcroutechoice,bctheorem,bcdesign,text width=2.55cm,minimum width=3.15cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N07}}}] (compchoice) at (9.35,-5.35) {\textbf{Routing theorem}\\which separation data exists?};
\node[bcbox,bclemma,bcdesign,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N08}}}] (typelemma) at (9.35,-8.25) {\textbf{Payload lemma}\\build \(P_{17\to3}\)\\type separation};
\node[bcbox,bclemma,bcdesign,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N09}}}] (labellemma) at (13.85,-8.25) {\textbf{Payload lemma}\\build \(P_{17\to10}\)\\label separation};
\node[bcbox,bclemma,bccheck,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N10}}}] (finite) at (-5.55,-11.20) {\textbf{Lemma}\\finite survivor-list certificate};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N11}}}] (typeroute) at (9.35,-11.10) {\textbf{CT 3}\\external types};
\node[bcroute,bcrouteneutral,bcdesign,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N12}}}] (labelroute) at (13.85,-11.10) {\textbf{CT 10}\\refinement};
\node[bcdec,bclemma,text width=3.00cm,minimum width=3.75cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N13}}}] (empty) at (-5.55,-14.30) {\textbf{Lemma}\\is the finite survivor list empty?};
\node[bcclosed,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N14}}}] (targetclose) at (-8.85,-17.60) {\textbf{C5}\\finite survivors exhausted};
\node[bcbox,bclemma,bcpayload,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N15}}}] (finitelemma) at (-3.00,-17.60) {\textbf{Payload lemma}\\build \(P_{17\to8}\)\\finite survivors};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N16}}}] (finitefig) at (-3.00,-20.60) {\textbf{CT 8}\\pumping};
\node[bcbox,bclemma,text width=3.35cm,minimum width=3.35cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N17}}}] (orbit) at (5.55,-11.20) {\textbf{Lemma}\\orbit, interval, or order criterion};
\node[bcdec,bclemma,text width=3.00cm,minimum width=3.75cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N18}}}] (arithq) at (5.55,-14.30) {\textbf{Lemma}\\does the arithmetic criterion rule out all blocks?};
\node[bcclosed,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N19}}}] (arithclose) at (3.00,-17.60) {\textbf{C1}\\arithmetic target hit};
\node[bcbox,bclemma,bcpayload,text width=3.10cm,minimum width=3.10cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N20}}}] (masslemma) at (8.85,-17.60) {\textbf{Payload lemma}\\build \(P_{17\to14}\)\\arithmetic mass};
\node[bcroute,bcrouteneutral,bcoutputroute,text width=3.00cm,minimum width=3.00cm,label={[font=\tiny,inner sep=1pt]north west:{\texttt{CT17.N21}}}] (arith) at (8.85,-20.60) {\textbf{CT 14}\\aggregate closure};
...
\draw[bcrecoverarr] (typelemma)--node[bclabel,right]{S-Trig for CT3}(typeroute);
\draw[bcrecoverarr] (labellemma)--node[bclabel,right]{S-Trig for CT10}(labelroute);
...
\draw[bchandoff] (finitelemma)--node[bclabel,right]{S-Trig for CT8}(finitefig);
...
\draw[bchandoff] (masslemma)--node[bclabel,right]{S-Trig for CT14}(arith);
\end{tikzpicture}%
}
...
\item
  ...
  generates its first-failure index (Sections \secref{the-activedormant-dichotomy} and \secref{structural-invariants-of-the-method}).
\item
...
\textbf{Boundaried piece} (\emph{piece}) & A bounded subobject
\((X,\partial X,\text{interface data})\) of \(G\), analyzed together
with its interface; \emph{proper} if \(X \ne G\). & Boundaried graph;
protrusion \\
\textbf{Branch state} & The accumulated logical context of a case:
baseline hypotheses, minimality, all previous branch failures, and the
current residual condition. & Case with recorded hypotheses \\
...
Myhill--Nerode class; type (logic) \\
...
\textbf{Residual} & The sub-case of a branch surviving after all named
exits are excluded. A \textbf{fully decorated residual} is an unresolved
package carrying the piece, branch state, absent exits, and relevant
minimality, carrier, or type data; it is terminal only after a valid
C1--C5 result. & --- \\
