...
/**
 * Return the compiled obligation ledger for one original manuscript cell.
 * When Lean does not export a property-level ledger, project the whole diagram
 * cell as exactly one task. This projection reports status; it never determines
 * whether the cell is formalized.
 */
export function proofFlowNodeObligations(
  ...
  const leanOwned = (manuscript.nodeObligations ?? []).filter(
    (obligation) => obligation.nodeId === nodeId,
  );
  if (leanOwned.length > 0) return leanOwned;

  const implementedSteps = manuscript.proofSteps.filter((step) =>
  ...
  const cellStatus: ErdosNodeObligationStatus = manuscript.formalizedNodeIds.includes(nodeId)
    ? "proved"
