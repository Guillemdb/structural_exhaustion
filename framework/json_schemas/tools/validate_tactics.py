...
def validate_tactic(t):
    ...
    semantic=t.get('graphVersion') is not None
    ...
    for i,n in enumerate(nodes,1):
        ...
        if not semantic and n['nodeId']!=expected: errors.append(f'{tid}: expected {expected}, got {n["nodeId"]}')
        if semantic and not n['nodeId'].startswith(f'{tid}.'): errors.append(f'{tid}: foreign semantic node ID {n["nodeId"]}')
        if semantic and n.get('runtimeKind') not in {'entry','decision','certification','terminal'}: errors.append(f'{n["nodeId"]}: missing runtimeKind')
    ...
    for n in nodes:
        ...
        if semantic and n['runtimeKind']=='terminal' and out[n['nodeId']]:
            errors.append(f'{n["nodeId"]}: terminal runtime node has outgoing transitions')
        if semantic and n['runtimeKind']!='terminal' and not out[n['nodeId']]:
            errors.append(f'{n["nodeId"]}: nonterminal runtime node is a sink')
        if semantic and n['runtimeKind']=='certification':
            if len(out[n['nodeId']]) != 1:
                errors.append(f'{n["nodeId"]}: certification node must have one successor')
            elif not out[n['nodeId']][0].get('transitionConstructor') or not out[n['nodeId']][0].get('evidenceSlots'):
                errors.append(f'{n["nodeId"]}: certification edge must carry the certified state')
    ...
    for n in nodes:
        if n['nodeType']=='decision':
            ...
            if semantic:
                by_edge={b['transitionId']:b for b in n.get('branches',[])}
                for e in out[n['nodeId']]:
                    branch=by_edge.get(e['transitionId'],{})
                    if not e.get('branchTag'): errors.append(f'{e["transitionId"]}: missing branchTag')
                    if not e.get('resultConstructor'): errors.append(f'{e["transitionId"]}: missing resultConstructor')
                    if not e.get('transitionConstructor'): errors.append(f'{e["transitionId"]}: missing transitionConstructor')
                    if e.get('branchTag')!=branch.get('guard'): errors.append(f'{e["transitionId"]}: branch tag differs from node guard')
                    if e.get('resultConstructor')!=branch.get('resultConstructor'): errors.append(f'{e["transitionId"]}: result constructor differs from node branch')
                    if e.get('evidenceSlots',[])!=branch.get('evidenceSlots',[]): errors.append(f'{e["transitionId"]}: evidence slots differ from node branch')
    ...
    if semantic:
        entries={n['nodeId'] for n in nodes if n['runtimeKind']=='entry'}
        if entries!=set(t['entryNodeIds']): errors.append(f'{tid}: runtime entry nodes differ from entryNodeIds')
        terminals={n.get('terminalCase') for n in nodes if n['runtimeKind']=='terminal'}
        if None in terminals: errors.append(f'{tid}: terminal runtime node missing terminalCase')
        totality=t.get('totality')
        if not totality: errors.append(f'{tid}: semantic graph missing totality declaration')
        elif terminals!=set(totality['terminalCases']): errors.append(f'{tid}: totality terminal cases differ from runtime terminals')
        for e in out[next(iter(entries), '')]:
            if not e.get('transitionConstructor'): errors.append(f'{e["transitionId"]}: entry edge missing transitionConstructor')
    return errors
...
def main(path):
    ...
    print(f'OK: {len(tactics)} tactics, {sum(len(t["nodes"]) for t in tactics)} nodes, {sum(len(t["transitions"]) for t in tactics)} transitions')
    return 0
