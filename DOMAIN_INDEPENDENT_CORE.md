...
Some manuscript nodes only derive one proposition from facts already
queryable on the active predecessor. They do not inspect new finite data and
do not introduce a mathematical branch. `Core.Residual.ProofProjection`
provides the canonical executor for this case. The application supplies the
literal predecessor focus, a dependent claim, and one exact `ActiveQuery`
proving that claim. Core privately packages the certificate, extends only the
active branch, preserves inactive siblings and the complete predecessor, and
publishes latest-certificate and latest-claim queries. Its primitive check
count is exactly the inherited focus-selection count; the proposition
projection itself adds zero checks. Every `Residual.Focus.Profile` therefore
contains a counted selector and a polynomial selection budget. Its constructor
is private: applications can use the zero-cost always-active focus or focuses
generated by framework decisions, but cannot install an opaque selector with a
fabricated count. Any executor that publishes work evidence uses
`Focus.runCounted`, whose stage and check count come from the same selector
evaluation and cover both active and inactive outcomes.

`Residual.Focus.Refinement` is the only generic way to narrow an existing
focus without adding an application decision stage, and its constructor is
private. The public `ActiveQuery.tagEqualTo` and `ActiveQuery.equalTo`
constructors compare a finite tag read from the exact active predecessor with
a fixed expected tag; Core owns the decision and its one-inspection budget.
`Focus.refine` evaluates the parent selector once and evaluates the child
comparison only on an active parent. Its independently specified dynamic
selection budget therefore records parent-plus-child work on active
predecessors and only parent work on inactive predecessors, while retaining
the combined polynomial envelope. `ActiveQuery.selectedTag` returns the exact
inherited value together with the selected equality from that same read;
`ActiveQuery.narrow` and `ActiveQuery.refinementProof` preserve more general
parent-owned reads without copying them into a new payload. Applications
cannot install an opaque child selector or attach their own count.

An application must not replace a proof projection with an application-owned
output structure, manually extend the ledger, copy the queried predecessor
fact into a new payload, or fabricate a one-arm route. If deriving the claim
requires inspecting candidates or deciding alternatives, the node must use
the corresponding CT or decision executor instead.
...
`TargetComplete` is a separate capability, not a label inferred from the
finite schedule. It proves that every nonnull class of the current quotient
has a target-visible representative in the exact predecessor-owned family.
Core then proves finite `AvoidsTargetVisible` if and only if literal
whole-quotient `BoundaryZero`. Without this capability, an exhaustive miss
only closes the listed family and cannot be promoted to quotient vanishing.
...
- counted refinement of an existing focus and inherited active queries;
